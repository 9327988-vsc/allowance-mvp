...
### 6.1 화면 인벤토리 (총 24개 화면 + Splash)

> v3.6 (HI4-01) 정정: 이전까지 헤딩이 "총 19개"라 적혀 있었으나 실제 카운트는 다음과 같음:
> - L1: 1개 (S-001)
> - L2: 14개 (S-101~109, S-110, S-111~114) — v3.4에서 S-110 신설
> - L3: 3개 (S-201~203)
...
> **= 총 24개 화면 + Splash = 25개**
>
> 결번 없음. v3.4까지는 17개 → v3.4 보완으로 S-110 추가 + Splash 신설로 19개로 늘렸어야 했으나 카운트 미정정. v3.6에서 정확한 수치로 갱신.
...
| 탭/클릭 동작 | 행 자체는 클릭 가능하지만 클릭 시 S-303 토스트 "🏫 학교/학원 단가는 ⚙ 설정에서 변경하세요" 표시 (4초, v3.6 ME4-01: 7.10 매트릭스와 일관) |
...
> **📍 7장에 명세 없는 화면 안내 (v3.6 ME4-02)**:
> 다음 3개 화면은 **관리자 모드** 전용 화면이라 **9장 (1단계 관리자/진단 모드)**에 명세되어 있습니다:
> - **S-113 데이터 내보내기** → 9.4
> - **S-114 데이터 가져오기** → 9.5
> - **S-203 카테고리 관리** → 9.6
>
...
| BTN-CM-004 | S-203 인라인 | [취소] | 인라인 편집 모드 종료 (변경 폐기) (v3.6 HI4-03) | - | - |
| BTN-CM-005 | S-203 삭제 모달 | [삭제] | `deleteCustomCategory(id)` → S-301 "✅ 카테고리 삭제됨" → 모달 닫기 (v3.6 HI4-03) | - | - |
| BTN-CM-006 | S-203 삭제 모달 | [취소] | 모달 닫기 (변경 없음) (v3.6 HI4-03) | - | - |
