...
| CR-02 | 7.13 신설, 8.1 | S-110 화면 명세 신설 (영역·버튼·동작·BTN-X-601) |
| CR-03 | 7.14 신설, 10.2 | Splash 화면 명세 신설 + 폴더 구조에 `Splash.jsx` 추가 |
...
| HI-11 | 7.13(S-114) | 가져오기 모드 선택 UI (덮어쓰기/병합 라디오) + 위험 안내 |
...
#### 중첩 모달 ESC 동작 매트릭스 (v3.2 추가, v3.4 ME-09 토스트 행 추가)
...
| **토스트만 단독** | **동작 없음** (토스트는 ESC로 안 닫힘) | - | 자동 사라짐만 (3초) |
...
#### Calendar 손상 시 사용자 알림 (v3.4 HI-09 신설)
...
2. 손상 콜백(`registerCorruptedCallback`) 트리거 → S-303 경고 토스트 표시:
   ...
   - 지속: 5초 (일반 토스트 3초보다 길게)
   - 액션 (선택): [관리자 모드 열기] 버튼 (`?admin=1` URL 이동)
...
useEffect(() => {
  registerCorruptedCallback((key) => {
    ...
    // settings_v1 손상은 부팅 단계에서 S-109로 처리됨
  });
}, []);
```
...
#### S-110 (스토리지 비활성 안내) — v3.4 신설 (CR-01, CR-02)
...
### 7.13 Splash (부팅 중 표시) — v3.4 신설 (CR-03)
...
│   │   │   ├── StorageDisabledModal.jsx # S-110 (v3.4 신설)
