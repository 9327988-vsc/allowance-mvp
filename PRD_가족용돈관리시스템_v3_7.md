...
## v3.7 변경 사항 (5차 나노 단위 전수조사 후 보완)

> **5차 보완 — 13건**: v3.6의 4차 보완(8건) 후속 점검에서 발견된 "약속 외 영역" 누락을 모두 처리.
> - 🔴 Critical 1건 (CR5-01): S-114 부속 확인 모달이 흐름에만 등장하고 인벤토리·매트릭스에 정의 없음 → S-115 신설로 일관화
> - 🟠 High 7건 (HI5-01~07): S-114 5개 버튼 ID, S-203 정리 모달 [취소] ID, 관리자 [정리]·[초기화] 확인 모달 명세, 8.1 빈 활성/비활성 조건, S-113 도해 [×], S-103 자동 항목 행 BTN ID
> - 🟡 Medium 5건 (ME5-01~05): Z-Order 액션 토스트 행, 정렬 패턴 안내 7.11 추가, 결번 정의 명확화, S-114 라디오 기본값 도해 정정, BTN-X-301·BTN-N-001~003 분리
> - 원칙 유지: **반드시 필요한 경우 외 화면·버튼·기능 신규 추가 금지**. 5차 보완은 신규 화면 3개(S-115/S-116/S-117 — 모두 v3.4 이전부터 흐름에 호출되어 있었으나 명세 누락된 부속 확인 모달), 신규 함수 0개. 기존 누락 ID 14개 부여.

### 5차 보완: 🔴 Critical 1건
| ID | 영역 | 수정 |
|---|---|---|
| CR5-01 | 6.1, 6.3, 6.4, 7.15 신설, 9.5 | S-114 "한 번 더 확인 모달" → S-115로 화면 ID 부여 + 7.15 신설 + 인벤토리·닫기 정책·ESC LIFO·Z-Order 4곳 동시 등재 |

### 5차 보완: 🟠 High 7건
| ID | 영역 | 수정 |
|---|---|---|
| HI5-01 | 8.1 | S-114 5개 버튼 ID 부여 (BTN-IM-001~005) |
| HI5-02 | 8.1, 9.6 | S-203 정리 모달 [취소] BTN-CM-008 추가 (HI4-03 약속한 7개 → 실제 8개로 정정) |
| HI5-03 | 8.1, 9.2 | BTN-A-005 [오래된 데이터 정리] 확인 모달 명세 (S-116 부여) + BTN-CL-001/002 |
| HI5-04 | 8.1, 9.2 | BTN-A-006 [모든 데이터 초기화] 2단계 type-to-confirm 모달 명세 (S-117 부여) + BTN-RS-001~004 |
| HI5-05 | 8.1 | 카탈로그 빈 활성/비활성 조건 일괄 채우기 ("항상 활성" 명시 또는 실제 조건 반영) |
| HI5-06 | 9.4 | (5차 보고서 오진단) S-113 ASCII 도해는 v3.5 시점부터 이미 헤더에 `[×]` 포함되어 있었음. 추가 수정 불필요. v3.7 변경표에는 검증 결과 기록 |
| HI5-07 | 8.1, 7.3 | BTN-CE-006 [자동 항목 행] (S-103) 추가 — 7.10 자동 항목 토스트 트리거 위치 명시 |

### 5차 보완: 🟡 Medium 5건
| ID | 영역 | 수정 |
|---|---|---|
| ME5-01 | 6.4 | Z-Order에 "Toast (액션 있음)" 행 별도 명시 (z=500 동일, 동작만 구분) |
| ME5-02 | 7.11 | 정산표 정렬 안내에 4.3 messageTemplate.js 동일 패턴 참조 1줄 추가 |
| ME5-03 | 6.1 | "결번 없음" 정의 명확화 ("사용 ID 사이에 빠진 번호 없음" + 미할당은 별개) |
| ME5-04 | 9.5 | S-114 ASCII 라디오 도해에서 "병합" 기본 선택 표시 (🔘 vs ⚪) |
| ME5-05 | 8.1 | 묶음 BTN ID 분리 — BTN-X-301 → 301/302/303, BTN-N-001~003 한 행 → 3행 |

### v3.6 → v3.7 분량 변화
| 항목 | v3.6 | v3.7 |
|---|---:|---:|
| 전체 | 약 6,092줄 / 약 246KB | 약 6,376줄 / 약 285KB (+5%) |
| 보완 합계 | 77건 (1차 20 + 2차 29 + 3차 20 + 4차 8) | + 13건 (5차) = 90건 |
| 신규 화면 | 0개 | 3개 (S-115 가져오기 덮어쓰기 확인 + S-116 오래된 데이터 정리 확인 + S-117 모든 데이터 초기화 — 흐름에 이미 호출되어 있던 미명세 모달 보강) |
| 신규 버튼 ID | BTN-CM-001~007 (7개) | + BTN-IM-001~006, BTN-CM-008, BTN-CL-001~002, BTN-RS-001~004, BTN-CE-006, BTN-X-302/303/402/502 (15개) |

---

## v3.6 변경 사항 (이전 변경 이력)

> **4차 보완 — 8건**: v3.5의 누적 69건 보완 후 후속 점검에서 발견된 세부 카운트·미세 ID 누락·중복 정의 의심 문제를 모두 처리.
> - 🔴 Critical 1건 (CR4-01): ExtraItem 데이터 모델 vs 코드 사용 패턴 명확화
> - 🟠 High 4건 (HI4-01~04): 인벤토리 헤딩 카운트, S-403 반응형 누락, S-203 7개 버튼 ID, Toast.jsx 중복 정리
> - 🟡 Medium 3건 (ME4-01~03): 토스트 duration 일관성, 7장 끝 안내, F1-3 표현
> - 원칙 유지: **반드시 필요한 경우 외 화면·버튼·기능 신규 추가 금지**. 4차 보완은 신규 화면 0개, 신규 함수 0개, 기존 명세 정확화만.

### 4차 보완: 🔴 Critical 1건
| ID | 영역 | 수정 |
|---|---|---|
| CR4-01 | 3.2, F1-3, 4.3 | `ExtraItem` 인터페이스 옆 노트 추가 + F1-3 정렬 표현 명확화 ("부모 셀의 date, 변환 시점에 부여") + 4.3 변환 코드 주석 |

### 4차 보완: 🟠 High 4건
| ID | 영역 | 수정 |
|---|---|---|
| HI4-01 | 6.1 | 인벤토리 헤딩 "총 19개" → "총 24개 화면 + Splash" + 비고 정정 |
| HI4-02 | 8.5 | 반응형 매트릭스에 S-403 (빈 상태) 행 추가 |
| HI4-03 | 8.1, 9.6 | S-203 7개 버튼 ID 부여 (BTN-CM-001 ~ BTN-CM-007) — 카탈로그 일관성 |
| HI4-04 | 10.2, 4.14 | `Toast.jsx` 폴더 구조에서 제거 (`ToastContainer.jsx`로 단일화) + 4.14 코드 주석 명확화 |

### 4차 보완: 🟡 Medium 3건
| ID | 영역 | 수정 |
|---|---|---|
| ME4-01 | 7.10 | 자동 항목 클릭 안내 토스트 duration 3초 → 4초 (S-303 기본값과 일관) |
| ME4-02 | 7장 끝 | "S-113·S-114·S-203은 9장 참조" 안내 1줄 추가 |
| ME4-03 | F1-3 | "extra_item.date" 표현 명확화 (CR4-01과 통합) |

### v3.5 → v3.6 분량 변화
| 항목 | v3.5 | v3.6 |
|---|---:|---:|
| 전체 | 약 5,981줄 / 약 245KB | 약 6,000줄 / 약 246KB (+0.5%) |
| 보완 합계 | 69건 (1차 20 + 2차 29 + 3차 20) | + 8건 (4차) = 77건 |
| 신규 화면 | 0개 | 0개 |
| 신규 함수 | 0개 | 0개 |
| 신규 버튼 ID | BTN-EX-001~003 (3개) | + BTN-CM-001~007 (7개) |
...
// ⚠ 데이터 모델 노트 (v3.6 CR4-01 명확화):
// ────────────────────────────────────────────────────────────
// ExtraItem 자체는 `date` 필드를 가지지 않습니다.
// 각 항목의 발생 날짜는 부모 객체의 키 `cells[date]`에서 파생됩니다.
//
// 따라서 정렬·표시 시점에 date가 필요하면 변환 패턴을 사용:
//   const allExtras = [];
//   Object.entries(calendar.cells).forEach(([date, cell]) => {
//     cell.extra_items.forEach(item => {
//       allExtras.push({ ...item, date });   // 임시 부여
//     });
//   });
//   allExtras.sort((a, b) => a.date.localeCompare(b.date));
//
// 이 패턴은 4.3 messageTemplate.js, 7.11 정산표 SummaryTable.jsx에서 사용됩니다.
// F1-3·7.11에서 언급되는 "extra_item.date 정렬"은 위 변환 후의 임시 객체 기준입니다.
// ────────────────────────────────────────────────────────────
...
export function generateMessage(year, month, calc, settings, holidays, customCategories = []) {
  ...
  // 임시 항목 정렬 (v3.6 CR4-01 명확화)
  // ────────────────────────────────────────────────────────────
  // ExtraItem 인터페이스(3.2)에는 date 필드가 없음.
  // 부모 셀의 date를 변환 시점에 임시 부여 → 1차 정렬 키로 사용.
  // ExtraItem.created_at는 인터페이스에 실제 존재 → 2차 정렬 키.
  // 이 정렬 순서는 7.11 정산표와 동일 (F1-3 ME-05 통일 정책).
  // ────────────────────────────────────────────────────────────
  const allExtras = [];
  calc.cells.forEach(c => {
    c.extra_items.forEach(item => {
      allExtras.push({ ...item, date: c.date });   // date는 임시 부여
    });
  });
  // 1차: date asc / 2차: created_at asc (같은 date 내)
  allExtras.sort((a, b) => {
 ...
 * @param {{ dryRun?: boolean }} [opts] - dryRun=true이면 삭제 없이 대상 목록만 반환 (v3.7 B2-02: S-116 미리보기용)
...
> **단일 파일 정책 (v3.6 HI4-04 명시)**: 토스트 관련 컴포넌트는 **이 `ToastContainer.jsx` 한 파일에 통합**되어 있습니다. v3.5까지 폴더 구조에 `Toast.jsx`도 있었으나 v3.6에서 제거 — 컨테이너가 토스트 div를 직접 inline 렌더링하므로 별도 Item 컴포넌트 불필요. 1단계 단순화 정책.
...
export default function ToastContainer() {
  ...
  return (
    <div className="toast-container" role="region" aria-label="알림">
      {toasts.map(t => (
        // 개별 토스트는 inline 렌더 (별도 Toast 컴포넌트 import 없음)
...
2. 표시할 행 결정 (고정 순서, v3.4 확정):
   ...
   4. 임시 항목 카테고리별 그룹 — **날짜 → 등록 시간 순서 (v3.4 ME-05 통일, v3.6 CR4-01 명확화)**
      - 1차 정렬: **부모 셀의 date** (asc) — `cells[date]` 키에서 파생. ExtraItem 자체는 date 필드를 가지지 않음 (3.2 인터페이스 노트 참조). 변환 패턴: `{...item, date}`
      - 2차 정렬: `extra_item.created_at` (asc) — 같은 날짜 내 등록 시간 오름차순 (이 필드는 ExtraItem 인터페이스에 실제 존재)
...
### 6.1 화면 인벤토리 (총 27개 화면 + Splash)

> v3.7 (CR5-01·HI5-03·HI5-04 + ME5-03) 정정: v3.6에서 "총 24개 화면 + Splash"였으나 다음 3개 미명세 모달을 추가 등재:
> - **S-115** 가져오기 덮어쓰기 확인 (S-114에서 호출되는 부속 모달, v3.4 시점부터 흐름에 호출되어 있었으나 명세 누락)
> - **S-116** 오래된 데이터 정리 확인 (BTN-A-005에서 호출, "확인 → cleanupOldCalendars"로만 표기되어 있던 것)
> - **S-117** 모든 데이터 초기화 (BTN-A-006에서 호출, "2단계 type-to-confirm"으로만 표기되어 있던 것)
>
> 갱신된 카운트:
> - L1: 1개 (S-001)
> - L2: 17개 (S-101~109, S-110, S-111~117) — v3.7에서 S-115/S-116/S-117 신설
> - L3: 3개 (S-201~203)
> - L4: 3개 (S-301/S-302/S-303 — 1행에 묶음 표기되지만 실제 3개 화면)
> - L5: 3개 (S-401/S-402/S-403)
> - 부속: Splash (v3.4 신설)
>
> **= 총 27개 화면 + Splash = 28개**
>
> **결번 정의 (v3.7 ME5-03 명확화)**: "결번 없음"은 "사용 중인 ID 사이에 빠진 번호 없음"을 뜻함. S-118 이후, S-204 이후, S-902 등은 미래 확장용 미할당 영역으로 결번 아님.
...
| L2 | **S-115** | **가져오기 덮어쓰기 확인 (v3.7 신설)** | S-114 [가져오기] + 모드=overwrite + 기존 데이터 존재 |
| L2 | **S-116** | **오래된 데이터 정리 확인 (v3.7 신설)** | S-112 [🧹 오래된 데이터 정리] (BTN-A-005) |
| L2 | **S-117** | **모든 데이터 초기화 (v3.7 신설)** | S-112 [🗑 모든 데이터 초기화] (BTN-A-006), 2단계 type-to-confirm |
...
#### 관리자 모드 보조 흐름 (v3.5 ME3-04 신설, v3.7 부속 모달 분기 명시)
...
URL ?admin=1 (또는 window.openAdmin())
   ...
   S-112 진단 화면
       │
       ├─► [📤 데이터 내보내기]  ──► S-113 (옵션 선택 → 다운로드 → S-301)
       │
       ├─► [📥 데이터 가져오기]  ──► S-114 (파일 선택 + 모드)
       │                              │
       │                              ├─ 모드=병합 + [가져오기]   ──► importData("merge") → S-301
       │                              │
       │                              └─ 모드=덮어쓰기 + [가져오기]
       │                                 (기존 데이터 존재 시)    ──► S-115 가져오기 덮어쓰기 확인
       │                                                              │
       │                                                              ├─ [취소] → S-114로
       │                                                              │
       │                                                              └─ [삭제 후 가져오기]
       │                                                                  → importData("overwrite") → S-301
       │
       ...
       │                                       ├─► 행 [✏️] → 인라인 편집
       │                                       │
       │                                       ├─► 행 [🗑] → 삭제 확인 모달 (BTN-CM-005/006)
       │                                       │
       │                                       └─► [정리] → 정리 미리보기 모달 (BTN-CM-007/008)
       │
       ├─► [🧹 오래된 데이터 정리]  ──► S-116 정리 확인 모달 (v3.7)
       │                                  │
       │                                  ├─ [취소] → S-112로
       │                                  └─ [정리하기] → cleanupOldCalendars → S-301
       │
       ├─► [🗑 모든 데이터 초기화]  ──► S-117 1단계 안내 모달 (v3.7)
       │                                  │
       │                                  ├─ [취소] → S-112로
       │                                  └─ [백업했습니다, 계속] → S-117 2단계 type-to-confirm
       │                                                              │
       │                                                              ├─ [← 이전] → 1단계로
       │                                                              └─ [완전히 삭제] (입력값 일치)
       │                                                                  → resetAllData → S-301
...
| S-114 가져오기 | ✅ | ✅ | ❌ | 진행 중 닫기 방지. 덮어쓰기 모드에서 [가져오기] 클릭 시 → S-115로 분기 |
| **S-115 가져오기 덮어쓰기 확인** (v3.7 신설) | ❌ | ✅ (= 취소) | ❌ | 명시적 선택만 인정 (S-107과 동일 정책). [취소] = S-115만 닫고 S-114 유지 |
| **S-116 오래된 데이터 정리 확인** (v3.7 신설) | ❌ | ✅ (= 취소) | ❌ | 명시적 선택만 인정. 정리 미리보기 표시, [취소] = S-116만 닫고 S-112 유지 |
| **S-117 모든 데이터 초기화** (v3.7 신설) | ❌ | ✅ (= 취소) | ❌ | 2단계 type-to-confirm. 1단계 [취소] = S-112로, 2단계 [취소] = 1단계로 복귀 |
| S-203 카테고리 관리 | ✅ | ✅ | ✅ | 인라인 편집 중이면 편집 취소 후 닫기 |
| S-201 비고 | ✅ | ✅ | ✅ | + 모바일 스와이프 |
| S-202 월 선택 | - | ✅ | ✅ | 일반적으로 외부 클릭 |
| S-301~303 토스트 | ✅ | - | - | + 자동 닫힘 (액션 토스트는 수동 닫기) |

### 6.4 Z-Order

| Layer | z-index | 화면 | 비고 (v3.7 ME5-01) |
|---|---|---|---|
| Toast (액션 있음) | 500 | calendar 손상 알림 등 | duration=0, 수동 닫기. ESC 무시 |
| Toast (자동) | 500 | S-301/S-302/S-303 일반 | duration > 0, 자동 사라짐 |
| Modal-3 | 400 | S-105, S-107, **S-115**, **S-116**, **S-117** (v3.7 신설 모달은 다른 모달 위에 표시되는 확인 모달이므로 Modal-3 레이어) |
| Modal-2 | 300 | S-104 |
...
#### 중첩 모달 ESC 동작 매트릭스 (v3.2 추가, v3.4 ME-09 토스트 행 추가, v3.5 HI3-06 S-203 행 추가, v3.7 CR5-01 신설 모달 행 추가)
...
| **S-114 → S-115** (v3.7 CR5-01) | **S-115만** (= 취소) | ❌ | S-114 유지 (사용자가 모드 재선택 가능). 명시적 선택 정책 (S-107과 동일) |
| **S-112 → S-116** (v3.7 HI5-03) | **S-116만** (= 취소) | ❌ | S-112 유지. 정리 미리보기는 명시적 선택만 인정 |
| **S-112 → S-117 (1단계)** (v3.7 HI5-04) | **S-117 1단계만** (= 취소) | ❌ | S-112 유지 |
| **S-117 1단계 → S-117 2단계** (v3.7 HI5-04) | **2단계만** (= 1단계로 복귀) | ❌ | type-to-confirm 입력 폐기, 1단계 안내 화면으로 |
...
- **데이터 영구 손실 확인 모달(S-115/S-116/S-117) ESC = 취소** (v3.7 CR5-01) — S-107과 동일 정책. 실수로 데이터 삭제 방지가 최우선.
...
| 탭/클릭 동작 | 행 자체는 클릭 가능 (BTN-CE-006, v3.7 HI5-07: 8.1 카탈로그 등재). 클릭 시 S-303 토스트 "🏫 학교/학원 단가는 ⚙ 설정에서 변경하세요" 표시 (4초, v3.6 ME4-01: 7.10 매트릭스와 일관) |
...
| **자동 항목 클릭 안내** (v3.5 HI3-03, v3.6 ME4-01: 4초로 통일) | S-303 | "🏫 학교/학원 단가는 ⚙ 설정에서 변경하세요" | 4초 |
...
| **오래된 데이터 정리 성공** (v3.7 B2-01) | S-301 | "✅ {N}개월 캘린더가 정리되었습니다" | 3초 |
| **오래된 데이터 정리 대상 없음** (v3.7 B2-01) | S-303 | "정리할 데이터가 없습니다" | 4초 |
| **모든 데이터 초기화 성공** (v3.7 B2-01) | S-301 | "✅ 모든 데이터가 초기화되었습니다" | 3초 |
...
**행 순서 (v3.4 확정, ME-05 통일, v3.7 ME5-02 참조 명시)**: 💰 기본 용돈 → 🏫 학교 → 📚 학원 → 임시 항목 (날짜 asc → 같은 날짜는 created_at asc) → 합계. 순서는 고정. 메시지 템플릿(4.3)과 동일 정렬.

> **정렬 구현 패턴 (v3.7 ME5-02)**: 정산표(SummaryTable.jsx)에서 임시 항목을 정렬할 때는 4.3 `messageTemplate.js`의 `allExtras` 변환 패턴을 그대로 사용. 즉, `cells.forEach` 루프에서 `{ ...item, date }` 형태로 임시 객체에 date를 부여한 뒤 `localeCompare`로 정렬. ExtraItem 자체에는 date 필드가 없으므로(3.2 인터페이스 + 4.3 코드 주석 참조, CR4-01) 변환 단계가 필수.
...
> **📍 7장에 명세 없는 화면 안내 (v3.6 ME4-02, v3.7 갱신)**:
> 다음 화면들은 **관리자 모드** 전용 화면이라 **9장 (1단계 관리자/진단 모드)**에 명세되어 있습니다:
> - **S-113 데이터 내보내기** → 9.4
> - **S-114 데이터 가져오기** → 9.5
> - **S-203 카테고리 관리** → 9.6
>
> v3.7에서 신설된 부속 확인 모달 3개는 7장에 명세 (S-115는 S-114에서, S-116·S-117은 S-112에서 호출되는 부속이지만 일반 모달 정책이 적용되므로):
> - **S-115 가져오기 덮어쓰기 확인** → 7.15
> - **S-116 오래된 데이터 정리 확인** → 7.16
> - **S-117 모든 데이터 초기화** → 7.17
>
> 6.1 인벤토리에는 모든 화면이 등재되어 있지만 명세 위치는 일반 사용자 화면(7장)과 관리자 화면(9장)으로 분리되어 있습니다. 6.2 흐름도의 "관리자 모드 보조 흐름" 섹션도 참조.

### 7.15 S-115 (가져오기 덮어쓰기 확인) — v3.7 신설 (CR5-01)

v3.4 시점부터 S-114 흐름(9.5 line 5142 부근)에 호출되어 있었으나 화면 명세가 누락되어 있었음. v3.7에서 별도 모달로 정식 등재.

**진입 트리거**: S-114에서 사용자가 "덮어쓰기" 모드 선택 + [가져오기] 클릭 + **기존 데이터 존재**(`localStorage.getItem("settings_v1") !== null` 또는 calendar 1개 이상)일 때.

**닫기 정책**: ❌ X / ✅ ESC (= 취소) / ❌ 외부 클릭 — S-107과 동일 (명시적 선택만 인정, 데이터 영구 손실 방지).
...
| 버튼 | ID | 동작 |
|---|---|---|
| [취소] | BTN-IM-005 | S-115만 닫기 (S-114 유지). 사용자가 모드 재선택 또는 백업 후 재진입 가능 |
| [삭제 후 가져오기] | BTN-IM-006 | `importData(file, "overwrite")` 실행 → 성공: S-114·S-115 동시 닫기 + S-301 "✅ N개 항목을 가져왔습니다" + 메인 화면 갱신 / 실패: S-115 닫기 + S-114에 에러 표시 |

**진행 중 처리**: [삭제 후 가져오기] 클릭 후 `importData` 실행 중에는 두 버튼 모두 disabled + 스피너. 보통 1초 이내, 큰 데이터는 최대 3초.

**Z-Order**: Modal-3 (z=400). S-114(Modal-1, z=200) 위에 표시.

**연결 지점**:
- 9.5 S-114의 "화면 동작 흐름" 4a 단계에서 호출
- ESC LIFO 매트릭스: "S-114 → S-115" 행 (6.4)

### 7.16 S-116 (오래된 데이터 정리 확인) — v3.7 신설 (HI5-03)

BTN-A-005 [🧹 오래된 데이터 정리] 클릭 시 호출되는 확인 모달. v3.6까지 8.1 카탈로그에 "확인 → cleanupOldCalendars"로만 표기되어 있었던 미명세 화면.

**진입 트리거**: S-112 진단 화면에서 BTN-A-005 [🧹 오래된 데이터 정리] 클릭.

**닫기 정책**: ❌ X / ✅ ESC (= 취소) / ❌ 외부 클릭 — 명시적 선택만 인정.
...
**삭제 대상 미리보기 정책**: `cleanupOldCalendars(6, { dryRun: true })` 호출로 실제 삭제 없이 대상 목록만 조회하여 미리보기 표시 (v3.7 B2-02: 4.6 함수 시그니처에 `{ dryRun }` 옵션 추가 확정).

**버튼 동작**:

| 버튼 | ID | 동작 |
|---|---|---|
| [취소] | BTN-CL-001 | S-116만 닫기 (S-112 유지) |
| [정리하기] | BTN-CL-002 | `cleanupOldCalendars(6)` 실행 → S-301 "✅ {N}개월 캘린더가 정리되었습니다" → S-116 닫기 + S-112 [데이터 상태] 영역 갱신 |

**삭제 대상 0건일 때**: 모달 진입 전 dry-run에서 0건 확인되면 모달 표시 안 함, S-303 토스트 "정리할 데이터가 없습니다" (4초)로 종료.

**Z-Order**: Modal-3 (z=400). S-112(Modal-1, z=200) 위에 표시.

### 7.17 S-117 (모든 데이터 초기화) — v3.7 신설 (HI5-04)

BTN-A-006 [🗑 모든 데이터 초기화] 클릭 시 호출되는 **2단계 type-to-confirm 모달**. v3.6까지 8.1 카탈로그에 "2단계 type-to-confirm"으로만 표기되어 있었던 미명세 화면.

**진입 트리거**: S-112 진단 화면에서 BTN-A-006 [🗑 모든 데이터 초기화] 클릭.

**닫기 정책**: ❌ X / ✅ ESC (= 취소 또는 1단계 복귀) / ❌ 외부 클릭.
...
| 버튼 | ID | 동작 |
|---|---|---|
| [취소] | BTN-RS-001 | S-117 닫기 (S-112 유지) |
| [백업했습니다, 계속] | BTN-RS-002 | 2단계로 전환 |
...
| 버튼 | ID | 동작 |
|---|---|---|
| [← 이전] | BTN-RS-003 | 1단계로 복귀 (입력값 폐기) |
| [완전히 삭제] | BTN-RS-004 | `resetAllData()` 실행 → S-301 "✅ 모든 데이터가 초기화되었습니다" → S-117 닫기 + 앱 재진입 (`window.location.reload()`) |

**[완전히 삭제] 활성 조건**: 입력값이 정확히 `"초기화 동의"` (공백·글자 일치, 앞뒤 trim 후 비교)일 때만 활성. 그 외에는 disabled.

**입력 폼 동작**:
- 한 글자라도 다르면 [완전히 삭제] 비활성 + 입력 박스 테두리 빨강 + 인라인 메시지 "입력이 일치하지 않습니다"
- 정확히 일치하면 테두리 초록 + [완전히 삭제] 활성

**ESC 동작**: 1단계 ESC = 취소(S-112로), 2단계 ESC = 1단계로 복귀 (입력값 폐기). ESC LIFO 매트릭스 참조.

**Z-Order**: Modal-3 (z=400).
...
| BTN-CE-002 | S-103 | ✏️ | S-104 편집 | 항상 | - |
| BTN-CE-003 | S-103 | 🗑 | S-107 | 항상 | - |
| BTN-CE-004 | S-103 | 취소 | isDirty 검사 (있으면 S-106) | 항상 | - |
| BTN-CE-005 | S-103 | 저장 | save → 닫기 | dirty | clean |
| BTN-CE-006 | S-103 | 자동 항목 행 (🏫/📚) | S-303 토스트 "🏫 학교/학원 단가는 ⚙ 설정에서 변경하세요" 4초 (v3.7 HI5-07: 7.10 자동 항목 안내 토스트 트리거) | 학교/학원 행 표시 시 | 행 미표시 |
| BTN-EF-001 | S-104 | 카테고리 드롭다운 | 목록 표시 | S-105 미오픈 시 | S-105 진입 중 (D-023) |
| BTN-EF-002 | S-104 드롭다운 | + 새 카테고리 | S-105 | S-105 미오픈 시 | S-105 진입 중 |
| BTN-EF-003 | S-104 | 추가 / 저장 | 검증 → 부모 모달 갱신 | 검증 통과 | 검증 실패 |
| BTN-EF-004 | S-104 | 취소 | 닫기 | 항상 | - |
| BTN-NC-001 | S-105 | 이모지 그리드 | 선택 | 항상 | - |
| BTN-NC-002 | S-105 | 추가 | 검증 → 저장 | 검증 통과 | 검증 실패 |
| BTN-NC-003 | S-105 | 취소 | 닫기 | 항상 | - |
| BTN-S-001 | S-101/S-102 | 요일 토글 | 선택/해제 | 항상 | - |
| BTN-S-002 | S-101/S-102 | 편도/왕복 | 라디오 | 항상 | - |
| BTN-S-003 | S-101/S-102 | 공휴일 처리 | 라디오 | 항상 | - |
| BTN-S-004 | S-101/S-102 | 저장 | 검증 → 저장 | 검증 통과 | 검증 실패 |
| BTN-S-005 | S-102 | 취소 | isDirty 검사 (있으면 S-106) | 항상 | - |
| BTN-N-001 | S-201 | × 닫기 (v3.7 ME5-05 분리) | S-201 닫기 | 항상 | - |
| BTN-N-002 | S-201 | 외부 클릭 (v3.7 ME5-05 분리) | S-201 닫기 | 항상 | - |
| BTN-N-003 | S-201 | 아래 스와이프 100px+ (모바일, v3.7 ME5-05 분리) | S-201 닫기 | 모바일 (< 768px) | 데스크톱 |
| BTN-X-101 | S-106 | 계속 편집 | S-106 닫기 (= 부모 모달 유지) | 항상 | - |
| BTN-X-102 | S-106 | 닫기 | 변경 폐기 + 부모 닫기 | 항상 | - |
| BTN-X-201 | S-107 | 취소 | S-107 닫기 | 항상 | - |
| BTN-X-202 | S-107 | 삭제 | draft에서 제거 + S-107 닫기 | 항상 | - |
| BTN-X-301 | S-108 | 취소 (v3.7 ME5-05 분리) | S-108 닫기 (정리 안 함) | 항상 | - |
| BTN-X-302 | S-108 | 지금 정리 (v3.7 ME5-05 분리) | `cleanupOldCalendars(6)` → 자동 재시도 (v3.4 C4-08) | 항상 | - |
| BTN-X-303 | S-108 | 내보내기 후 정리 (v3.7 ME5-05 분리) | S-113 진입 → 다운로드 후 cleanupOldCalendars 실행 | 항상 | - |
| BTN-X-401 | S-109 | 다시 설정하기 | S-101 강제 진입 | 항상 | - |
| BTN-X-402 | S-109 | 복구 시도 (v3.7 누락 보강) | `recoverFromBackup("settings_v1")` 호출 → 성공 시 S-001 / 실패 시 에러별 토스트 (C4-09) | 백업 키 존재 시 | 백업 키 없을 때 |
| BTN-X-501 | S-111 | 모두 선택 | textarea 자동 선택 | 항상 | - |
| BTN-X-502 | S-111 | 닫기 (v3.7 ME5-05 분리) | S-111 닫기 | 항상 | - |
| BTN-X-601 | S-110 | 다시 시도 | isStorageAvailable() 재검사 → 활성 시 initApp() 재실행, 비활성 시 모달 유지 + S-303 | 항상 | - |
| BTN-A-001 | S-112 | 일반 모드로 돌아가기 | URL에서 `?admin=1` 제거 → S-001 | 항상 | - |
| BTN-A-002 | S-112 | 데이터 내보내기 | S-113 | 항상 | - |
| BTN-A-003 | S-112 | 데이터 가져오기 | S-114 | 항상 | - |
| BTN-A-004 | S-112 | 카테고리 관리 | S-203 | 항상 | - |
| BTN-A-005 | S-112 | 오래된 데이터 정리 | **S-116 표시** (v3.7 HI5-03 명세). 미리보기 후 정리 | 6개월 이전 캘린더 1개+ | 모두 6개월 이내 |
| BTN-A-006 | S-112 | 모든 데이터 초기화 | **S-117 표시** (v3.7 HI5-04 명세). 2단계 type-to-confirm | 항상 | - |
| BTN-A-007 | S-112 | 콘솔 로그 보기 | `console.log` 로 localStorage 전체 + `getSystemInfo()` 출력 → alert("F12 콘솔에서 확인") (v3.5 HI3-02) | 항상 | - |
| BTN-A-008 | S-112 | 무결성 검사 | `checkDataStatus()` (9.3) 호출 → 결과를 9.2 [데이터 상태] 영역에 즉시 갱신 (v3.5 HI3-02) | 항상 | - |
| BTN-A-009 | S-112 | 공휴일 검증 | `getHolidays()` 결과 키 개수 + 2026~2030 연도별 검증 → alert(`총 N개 공휴일 로드됨, 연도별: 2026=A건, 2027=B건...`) (v3.5 HI3-02) | 항상 | - |
| BTN-A-010 | S-203 | 사용 안 한 카테고리 정리 | 정리 대상 미리보기 모달 (BTN-CM-007/008) → 확인 → cleanupUnusedCategories() | 미사용 카테고리 존재 | 모두 사용 중 |
...
| BTN-EX-002 | S-113 | 취소 | 모달 닫기 | 항상 | - |
| BTN-EX-003 | S-113 | × 닫기 | 모달 닫기 | 진행 중 아닐 때 | 다운로드 진행 중 |
| BTN-IM-001 | S-114 | 파일 선택 (v3.7 HI5-01) | `<input type="file" accept=".json">` 트리거 → 자동 검증 (line 5142 단계 2) | 항상 | - |
| BTN-IM-002 | S-114 | 취소 (v3.7 HI5-01) | S-114 닫기 | 진행 중 아닐 때 | 가져오기 진행 중 |
| BTN-IM-003 | S-114 | 가져오기 (v3.7 HI5-01) | 모드=병합: `importData(file, "merge")` 즉시 실행. 모드=덮어쓰기 + 기존 데이터 존재: **S-115 표시** | 파일 검증 통과 + 모드 선택 | 검증 실패 또는 파일 미선택 |
| BTN-IM-004 | S-114 | × 닫기 (v3.7 HI5-01) | S-114 닫기 | 진행 중 아닐 때 | 가져오기 진행 중 |
| BTN-IM-005 | S-115 | 취소 (v3.7 CR5-01) | S-115만 닫기 (S-114 유지) | 항상 | - |
| BTN-IM-006 | S-115 | 삭제 후 가져오기 (v3.7 CR5-01) | `importData(file, "overwrite")` 실행 → 성공 시 S-114·S-115 닫기 + S-301 토스트 | 항상 | 진행 중 |
| BTN-CM-001 | S-203 | 행 [✏️] 수정 | 해당 행 인라인 편집 모드 진입 (다른 행 [✏️]/[🗑]/[+ 추가]/[정리] disabled) (v3.6 HI4-03) | 인라인 편집 중 아닐 때 | 다른 행 편집 중 |
| BTN-CM-002 | S-203 | 행 [🗑] 삭제 | 카테고리 삭제 확인 모달 표시 (사용 중이면 N건 경고) (v3.6 HI4-03) | 인라인 편집 중 아닐 때 | 다른 행 편집 중 |
| BTN-CM-003 | S-203 인라인 | [저장] | `validateCustomCategory()` → `updateCustomCategory(id, {name, icon})` → S-301 "✅ 카테고리 정보가 변경되었습니다..." (v3.6 HI4-03) | 검증 통과 | 검증 실패 / 미변경 |
| BTN-CM-004 | S-203 인라인 | [취소] | 인라인 편집 모드 종료 (변경 폐기) (v3.6 HI4-03) | 항상 | - |
| BTN-CM-005 | S-203 삭제 모달 | [삭제] | `deleteCustomCategory(id)` → S-301 "✅ 카테고리 삭제됨" → 모달 닫기 (v3.6 HI4-03) | 항상 | - |
| BTN-CM-006 | S-203 삭제 모달 | [취소] | 모달 닫기 (변경 없음) (v3.6 HI4-03) | 항상 | - |
| BTN-CM-007 | S-203 정리 모달 | [정리하기] | `cleanupUnusedCategories()` → S-301 "✅ {N}개 카테고리가 정리되었습니다" → 모달 닫기 (v3.6 HI4-03) | 미사용 카테고리 1건+ | - |
| BTN-CM-008 | S-203 정리 모달 | [취소] (v3.7 HI5-02) | 정리 모달만 닫기 (S-203 유지) | 항상 | - |
| BTN-CL-001 | S-116 | [취소] (v3.7 HI5-03) | S-116 닫기 (S-112 유지) | 항상 | - |
| BTN-CL-002 | S-116 | [정리하기] (v3.7 HI5-03) | `cleanupOldCalendars(6)` → S-301 "✅ {N}개월 캘린더가 정리되었습니다" → S-116 닫기 + S-112 갱신 | 정리 대상 1건+ | 정리 대상 0건 (S-116 진입 자체 차단) |
| BTN-RS-001 | S-117 1단계 | [취소] (v3.7 HI5-04) | S-117 닫기 (S-112 유지) | 항상 | - |
| BTN-RS-002 | S-117 1단계 | [백업했습니다, 계속] (v3.7 HI5-04) | 2단계로 전환 | 항상 | - |
| BTN-RS-003 | S-117 2단계 | [← 이전] (v3.7 HI5-04) | 1단계로 복귀 (입력값 폐기) | 항상 | - |
| BTN-RS-004 | S-117 2단계 | [완전히 삭제] (v3.7 HI5-04) | `resetAllData()` → S-301 "✅ 모든 데이터가 초기화되었습니다" → window.location.reload() | 입력값이 정확히 "초기화 동의" | 입력값 불일치 |
...
| S-115 (덮어쓰기 확인, v3.7 CR5-01) | 95% 폭 | 420px 모달 | 420px 모달 | 420px 모달 |
| S-116 (정리 확인, v3.7 HI5-03) | 95% 폭 (스크롤, 미리보기 길 수 있음) | 480px 모달 | 480px 모달 | 480px 모달 |
| S-117 (모든 데이터 초기화, v3.7 HI5-04) | 95% 폭 (1·2단계 동일) | 460px 모달 | 460px 모달 | 460px 모달 |
...
| S-403 (빈 상태, v3.6 HI4-02) | S-401 영역 인라인 교체 (2줄 메시지) | 동일 | 동일 | 동일 (사이드바에서도 동일 인라인) |
...
> **버튼 ID (8.1 카탈로그, v3.7 HI5-01)**: [×] = **BTN-IM-004**, [파일 선택] = **BTN-IM-001**, [취소] = **BTN-IM-002**, [가져오기] = **BTN-IM-003**. 덮어쓰기 분기에서 호출되는 S-115는 7.15 + BTN-IM-005/006 참조.
...
| 4a. 덮어쓰기 + 기존 데이터 존재 (v3.7 CR5-01 정식 명세) | **S-115 가져오기 덮어쓰기 확인 모달** 표시 (7.15 참조). 즉시 importData 실행 안 함. 사용자가 BTN-IM-006 [삭제 후 가져오기] 클릭 시에만 overwrite 실행. BTN-IM-005 [취소] 시 S-114로 복귀 |
...
#### [✏] 카테고리 수정 — 인라인 편집 (v3.4, v3.6 HI4-03 ID 부여)

> 버튼 ID (8.1 카탈로그): 행 [✏️] = **BTN-CM-001**, 인라인 [저장] = **BTN-CM-003**, 인라인 [취소] = **BTN-CM-004**

1. **BTN-CM-001 [✏]** 클릭 → 해당 행이 인라인 편집 모드로 전환
2. 이름: `<input>` (기존 값 pre-fill, maxLength=20)
3. 아이콘: 클릭 시 이모지 피커 팝오버 (S-105와 동일 COMMON_EMOJIS)
4. **BTN-CM-003 [저장]** / **BTN-CM-004 [취소]** 인라인 버튼
...
#### [🗑] 카테고리 삭제 — 확인 모달 (v3.4, v3.6 HI4-03 ID 부여)

> 버튼 ID (8.1 카탈로그): 행 [🗑] = **BTN-CM-002**, 모달 [삭제] = **BTN-CM-005**, 모달 [취소] = **BTN-CM-006**

1. **BTN-CM-002 [🗑]** 클릭 → 확인 모달 표시
...
#### [사용 안 한 사용자 카테고리 정리] — 확인 흐름 (v3.4 HI-02 신설, v3.6 HI4-03 ID 부여)

> 버튼 ID (8.1 카탈로그): [사용 안 한 카테고리 정리] = **BTN-A-010**, 정리 미리보기 모달 [정리하기] = **BTN-CM-007**
...
│   │   │   ├── ConfirmOverwriteModal.jsx # S-115 (v3.7 CR5-01 신설)
│   │   │   ├── CleanupConfirmModal.jsx  # S-116 (v3.7 HI5-03 신설)
│   │   │   └── ResetAllDataModal.jsx    # S-117 (v3.7 HI5-04 신설, 1단계+2단계 통합)
│   │   ├── drawers/
│   │   │   ├── NotesDrawer.jsx          # S-201
│   │   │   ├── MonthSelector.jsx        # S-202
│   │   │   └── CategoryManager.jsx      # S-203
│   │   ├── widgets/
│   │   │   ├── ToastContainer.jsx       # S-301/S-302/S-303 (v3.5 CR3-02 신설, v3.6 HI4-04: 단일 파일로 통합. Toast 렌더링은 이 파일이 inline으로 처리)
...
| D-037 (v3.6) | ExtraItem 인터페이스에 `date` 필드 추가하지 않음 | 부모 셀의 `cells[date]` 키에서 파생되는 정보. 중복 저장은 1단계 단순화 정책 위반. 변환 시점에 `{...item, date}`로 임시 부여하는 패턴이 충분히 명확 (CR4-01 처리) | date 필드 추가 → 데이터 중복, 동기화 부담, "정답 위치" 모호 |
| D-038 (v3.6) | Toast 관련 컴포넌트는 `ToastContainer.jsx` 단일 파일 | 컨테이너가 inline 렌더링 → 별도 Toast Item 컴포넌트 불필요. 1단계 단순화. HI4-04 처리 | Toast.jsx + ToastContainer.jsx 분리 → 코드 분량 증가, Item-Container 협업 패턴 명세 부담 |
| D-039 (v3.6) | S-203 카테고리 관리의 7개 버튼에 BTN-CM-001~007 ID 부여 | 다른 화면(S-103: BTN-CE-XXX, S-107: BTN-X-XXX)은 모든 버튼에 ID 부여하므로 일관성 필요. 키보드 접근성 자동 테스트·문서 추적성. HI4-03 처리 | ID 미부여 → 카탈로그 일관성 깨짐, 검색·테스트 도구가 ID 기반일 때 누락 |
| D-040 (v3.6) | 인벤토리 헤딩 카운트는 1행 1화면 기준이 아닌 "실제 화면 ID 수" 기준 | S-301~303은 1행에 묶음 표기되지만 실제로는 3개 화면. 카운트 정확성이 작업 분량 추정에 영향. HI4-01 처리 | 행 수 기준 → 묶음 표기로 인한 카운트 부정확 (v3.5까지 "19개"라 적혀있던 실제 24개 화면) |
| D-041 (v3.7) | 흐름에만 등장하던 부속 확인 모달 3개(S-115/S-116/S-117)에 정식 화면 ID 부여 | "약속 외 영역" 누락의 근본 원인 = 흐름 텍스트가 모달을 호출하면서 인벤토리에는 등재 안 함. 5차 보고서 CR5-01·HI5-03·HI5-04로 진단된 패턴. 정식 ID 부여로 닫기 정책·ESC LIFO·Z-Order 4중 SSoT 회복 | S-107 일반화 재사용 → 정책(메시지·버튼 라벨·에러 처리)이 화면별로 다르므로 단일화 부적합. 새 ID가 추적성·테스트·문서 검색에서 유리 |
| D-042 (v3.7) | S-117 [모든 데이터 초기화]는 type-to-confirm 입력값을 한국어 "초기화 동의"로 고정 | 영문 "DELETE"는 한국 사용자에게 어색·실수 위험. 한국어 짧은 문구가 의도 파악 명확. 8자 정도가 우발적 입력 방지에 충분. HI5-04 처리 | 영문 "DELETE" → 키보드 전환 부담, "삭제" 같은 짧은 단어 → 우발적 입력 위험 |
| D-043 (v3.7) | 모든 BTN ID에 활성/비활성 조건 명시 ("항상 활성" 표기 포함) | v3.6까지 빈 칸(`-`)이 "항상 활성"인지 "미정"인지 모호 → 구현자 임의 결정. 모든 행이 "항상" 또는 실제 조건을 표기하면 카탈로그가 진짜 SSoT가 됨. HI5-05 처리 | 빈 칸 유지 → 카탈로그가 부분적 SSoT, 화면 명세 본문과 어느 쪽이 정답인지 모호 |
| D-044 (v3.7) | 묶음 BTN ID(BTN-X-301, BTN-N-001~003)를 개별 행으로 분리 | 한 ID가 여러 동작을 묶고 있으면 키보드 접근성 자동 테스트(축적된 BTN ID 매핑)가 누락 발생. 검색 시 "BTN-X-302"가 안 잡힘. ME5-05 처리 | 묶음 유지 → 검색·테스트 도구 누락, 동작 명세가 카탈로그에서 1줄로 압축되어 누락 가능 |
| D-045 (v3.7) | S-115/S-116/S-117 모두 Modal-3 레이어(z=400) 배치 | 부모 모달(S-114=Modal-1, S-112=Modal-1) 위에 표시되는 부속 확인 모달은 Modal-3 (z=400). 기존 S-105·S-107도 동일 패턴(부모 모달 위 = z=400). 일관성 유지 | Modal-2 → S-104와 충돌 가능성 / Modal-1 → 부모 위에 표시 안 됨 |
...
| v3.5 | 2026-05-02 | 3차 나노 단위 전수조사 20건 전면 반영. 🔴 Critical 4건(CR3-01~04): `getPanelVariant()` inline 변경, `showToast()` Singleton API 명세 신설(4.14), S-203 [+ 새 카테고리 추가] BTN-A-011 + S-105 재사용 흐름, S-113 화면 UI 영역 신설(BTN-EX-001~003). 🟠 High 9건(HI3-01~09): 6.2 흐름도 보강, 9.2 디버그 3개 버튼 동작 명세, 7.10 토스트 매트릭스 11건 누락 등재 + 액션 토스트 패턴, App.jsx에 `registerCorruptedCallback` + `ToastContainer` 통합(4.10 단일 SSoT) 등. 🟡 Medium 5건(ME3-01~05). 자체 모순 정정 2건(NV3-01~02). 의사결정 로그 D-032~D-036. 신규 함수: `showToast`/`dismissToast` (toastManager.js). 신규 화면: 0개. |
| **v3.6** | **2026-05-02** | **4차 나노 단위 전수조사 8건 전면 반영. 🔴 Critical 1건(CR4-01): `ExtraItem` 인터페이스 옆 노트 + F1-3·4.3 정렬 표현 명확화 (date 필드는 부모 셀에서 파생). 🟠 High 4건(HI4-01~04): 6.1 인벤토리 헤딩 카운트 정정 (19→24+Splash), 8.5 반응형 매트릭스에 S-403 행 추가, S-203의 7개 버튼 ID 부여 (BTN-CM-001~007), Toast.jsx 폴더 구조에서 제거 (ToastContainer.jsx 단일화). 🟡 Medium 3건(ME4-01~03): 자동 항목 토스트 4초 통일, 7장 끝에 "S-113·S-114·S-203은 9장 참조" 안내, F1-3 표현 정정. 의사결정 로그 D-037~D-040. 신규 화면 0개, 신규 함수 0개 (기존 명세 정확화만). 누적 보완 77건.** |
| **v3.7** | **2026-05-02** | **5차 나노 단위 전수조사 13건 전면 반영. 🔴 Critical 1건(CR5-01): S-114 흐름에서 호출되던 "한 번 더 확인 모달"을 S-115로 정식 등재 + 7.15 신설 + 인벤토리·닫기 정책·ESC LIFO·Z-Order 4곳 동시 등재. 🟠 High 7건(HI5-01~07): S-114의 5개 버튼 ID 부여(BTN-IM-001~005), S-203 정리 모달 [취소] BTN-CM-008, BTN-A-005 [오래된 데이터 정리]를 S-116으로 정식 명세(BTN-CL-001~002), BTN-A-006 [모든 데이터 초기화]를 S-117 2단계 type-to-confirm으로 정식 명세(BTN-RS-001~004), 8.1 카탈로그 빈 활성/비활성 조건 일괄 채우기, S-103 자동 항목 행에 BTN-CE-006 부여, S-113 ASCII [×]는 v3.5부터 이미 있었음을 검증. 🟡 Medium 5건(ME5-01~05): Z-Order에 액션 토스트 행 추가, 7.11 정산표 정렬 패턴 4.3 참조, 결번 정의 명확화, S-114 라디오 기본값 도해 정정(🔘 병합), BTN-X-301·BTN-X-501·BTN-N-001~003 묶음 분리. 의사결정 로그 D-041~D-045 추가. 신규 화면 3개(S-115/S-116/S-117 — 모두 v3.4 이전부터 흐름에 호출되던 미명세 부속 모달). 신규 버튼 ID 14개. 신규 함수 0개. 누적 보완 90건.** |
