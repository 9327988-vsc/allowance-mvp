...
**대상 단계:** 1단계 ~ 4단계 통합 (1단계 화면·인터랙션 면밀화, 2~4단계 개요)

**v2.1 변경 사항 (이번 보강):**
- v2.0의 "두루뭉실한 화면·조회·설정" 영역을 면밀화
- **17개 화면 전체 인벤토리 작성** (기존 ~6개 → 17개)
- **화면 정보 구조 / IA 다이어그램 / 화면 전이 매트릭스** 추가
- **화면별 영역 분할 + 디자인 토큰 (CSS 변수) + ARIA 속성** 명세
- **새 화면 식별**: 변경 확인(S-106)·정리 안내(S-108)·손상 안내(S-109)·클립보드 폴백(S-111)·진단(S-112)·내보내기(S-113)·가져오기(S-114)·카테고리 관리(S-203)·셀 호버 툴팁(S-402)
- **관리자 / 진단 모드 신설**: 시스템 정보·데이터 상태 검사·백업·복원·통계
- **버튼 카탈로그 50+ 개** + 키보드 단축키 + 마우스/터치 인터랙션 매트릭스
- **데이터 변경 추적 (isDirty)·체크섬(SHA-256)·손상 감지+자동 백업** 로직
- 인수 조건 (Gherkin) 약 80건 → 130건
- TypeScript 의사 코드 약 30개 → 70개 함수

**v2.0 변경 사항:**
- 1단계 PRD를 **개발 착수 가능한 상세 수준**으로 재작성 (각 기능마다 사용자 스토리, 흐름, 검증, UI 구조, 데이터 처리, 인수 조건, 테스트 케이스 포함)
...
7. 단계별 개발 로드맵
8. 1단계 상세 명세 (MVP)
9. 1단계 화면 시스템 — v2.1 신규
10. 1단계 화면별 상세 명세 — v2.1 신규
11. 1단계 관리자 / 진단 기능 — v2.1 신규
12. 1단계 인터랙션 카탈로그 — v2.1 신규
13. v2.1 변경 이력 — v2.1 신규
14. 2단계 상세 명세 (Growth)
15. 3단계 상세 명세 (Account System)
16. 4단계 상세 명세 (Production)
17. 데이터 모델
18. 시스템 설계 (아키텍처)
19. 비기능 요구사항
15. 일정 및 마일스톤
...
## 9. 1단계 화면 시스템 (Screen System) — v2.1 신규 보강

### 9.1 화면 인벤토리 (Screen Inventory)

1단계의 모든 화면을 빠짐없이 식별하고 분류한다.

#### 9.1.1 화면 분류 체계

| 레벨 | 정의 | 예시 |
|---|---|---|
| **L1: 메인 화면** | 앱의 기본 화면 (URL 라우트 1개) | 메인 캘린더 |
| **L2: 모달** | 메인 화면 위에 떠 있는 별도 화면 | 셀 편집 모달, 설정 모달 |
| **L3: 드로어/바텀시트** | 슬라이드 패널 | 비고 드로어, 카테고리 드로어 |
| **L4: 토스트/스낵바** | 일시적 알림 | "복사되었습니다" |
| **L5: 인라인 영역** | 메인 화면 내 동적 영역 | 정산표, 셀 호버 툴팁 |

#### 9.1.2 화면 전체 목록 (총 17개)

##### 메인 화면 (L1)
| ID | 화면명 | 진입 경로 |
|---|---|---|
| **S-001** | 메인 캘린더 화면 | 앱 첫 진입 (settings_v1 있을 때) |

##### 모달 화면 (L2)
| ID | 화면명 | 진입 경로 |
|---|---|---|
| **S-101** | 환영 + 자녀 기본 설정 모달 (첫 사용) | 앱 첫 진입 (settings_v1 없을 때) |
| **S-102** | 자녀 기본 설정 변경 모달 (재설정) | [⚙ 설정] 버튼 |
| **S-103** | 셀 편집 모달 | 캘린더 셀 클릭 |
| **S-104** | 임시 항목 추가/수정 폼 (셀 편집 모달 내부) | [+ 임시 항목 추가] 버튼 |
| **S-105** | 사용자 정의 카테고리 추가 모달 | 카테고리 드롭다운 [+ 새 카테고리] |
| **S-106** | 변경 사항 확인 모달 | 저장 안 한 상태로 닫기 시도 |
| **S-107** | 임시 항목 삭제 확인 모달 | 임시 항목 [삭제] 클릭 |
| **S-108** | 데이터 정리 모달 (스토리지 가득) | 저장 실패 시 자동 |
| **S-109** | 데이터 손상 안내 모달 | settings 손상 감지 시 자동 |
| **S-110** | 메시지 미리보기 모달 (선택) | [미리보기] 버튼 (1단계 후반 추가) |
| **S-111** | 클립보드 폴백 모달 | 클립보드 API 미지원 시 |
| **S-112** | 관리자 진단 화면 (Diagnostic) | URL `?admin=1` 접근 |
| **S-113** | 데이터 내보내기 모달 | 진단 화면 내 [내보내기] |
| **S-114** | 데이터 가져오기 모달 | 진단 화면 내 [가져오기] |

##### 드로어/바텀시트 (L3)
| ID | 화면명 | 진입 경로 |
|---|---|---|
| **S-201** | 비고 드로어/바텀시트 | [📌 비고] 버튼 |
| **S-202** | 월 선택 드롭다운 | 캘린더 헤더 월 표시 영역 클릭 |
| **S-203** | 카테고리 관리 드로어 | 진단 화면 내 [카테고리 관리] |

##### 토스트/스낵바 (L4)
| ID | 화면명 | 트리거 |
|---|---|---|
| **S-301** | 성공 토스트 | 저장 성공, 복사 성공 등 |
| **S-302** | 에러 토스트 | 검증 실패, 저장 실패 등 |
| **S-303** | 경고 토스트 | 한도 도달 등 |

##### 인라인 영역 (L5)
| ID | 영역명 | 위치 |
|---|---|---|
| **S-401** | 정산표 | 메인 화면 하단 |
| **S-402** | 캘린더 셀 호버 툴팁 | 셀 위 (PC) / 길게 누름 (모바일) |
| **S-403** | 빈 상태 안내 (캘린더 비어있을 때) | 메인 화면 |

---

### 9.2 화면 정보 구조 (Information Architecture)
...
[앱 진입]
   │
   ├─ 설정 없음? ──► S-101 환영 + 자녀 기본 설정 (필수, 닫기 불가)
   │                  │
   │                  └─ [저장하고 시작하기] ──┐
   │                                         │
   └─ 설정 있음? ◄────────────────────────────┘
              ...
              S-001 메인 캘린더 화면
                  │
                  ├─ [◀ 이전 달] / [▶ 다음 달] ─► 월 변경 (같은 화면)
                  │
                  ├─ 월 표시 영역 클릭 ─► S-202 월 선택 드롭다운
                  │                          │
                  │                          └─ 월 선택 ─► 메인 화면 갱신
                  │
                  ├─ 캘린더 셀 클릭 ─► S-103 셀 편집 모달
                  │                       │
                  │                       ├─ [+ 임시 항목 추가] ─► S-104 임시 항목 폼
                  │                       │                            │
                  │                       │                            ├─ 카테고리 [+ 새 카테고리] ─► S-105
                  │                       │                            ├─ [추가] ─► S-103로 복귀
                  │                       │                            └─ [취소] ─► S-103로 복귀
                  │                       │
                  │                       ├─ 임시 항목 [수정] ─► S-104 (편집 모드)
                  │                       ├─ 임시 항목 [삭제] ─► S-107 삭제 확인 ─► S-103로 복귀
                  │                       │
                  │                       ├─ [저장] ─► S-001로 복귀 + S-301 성공 토스트
                  │                       └─ [취소] ─► (변경 있음) S-106 확인 ─► S-001
                  │                                   (변경 없음) S-001로 복귀
                  │
                  ├─ [⚙ 설정] ─► S-102 설정 변경 모달
                  │                  │
                  │                  ├─ [저장] ─► S-001로 복귀 + S-301 토스트
                  │                  └─ [취소] ─► S-001로 복귀
                  │
                  ├─ [📌 비고] ─► S-201 비고 드로어/바텀시트
                  │                 │
                  │                 └─ [×] / 외부 클릭 / ESC ─► S-001로 복귀
                  │
                  └─ [📋 메시지 복사]
                          │
                          ├─ 정상 ─► 클립보드 복사 + S-301 성공 토스트
                          └─ 실패 ─► S-111 클립보드 폴백 모달

[관리자 모드]
   │
   └─ URL ?admin=1 ─► S-112 진단 화면
                         │
                         ├─ [데이터 내보내기] ─► S-113
                         ├─ [데이터 가져오기] ─► S-114
                         ├─ [카테고리 관리] ─► S-203
...
각 화면에서 어디로 갈 수 있는지 매트릭스로 정리.

| 출발/도착 | S-001 | S-101 | S-102 | S-103 | S-104 | S-201 | S-301 | S-112 |
|---|---|---|---|---|---|---|---|---|
| **S-001 (메인)** | - | - | ⚙ | 셀클릭 | - | 비고 | 복사 | URL |
| **S-101 (첫설정)** | 저장 | - | - | - | - | - | - | - |
| **S-102 (재설정)** | 저장/취소 | - | - | - | - | - | 저장 시 | - |
| **S-103 (셀편집)** | 저장/취소 | - | - | - | +추가 | - | 저장 시 | - |
| **S-104 (임시폼)** | - | - | - | 추가/취소 | - | - | - | - |
| **S-201 (비고)** | 닫기 | - | - | - | - | - | - | - |
| **S-112 (관리자)** | 돌아가기 | - | - | - | - | - | - | - |
...
#### 9.3.1 모달 닫기 규칙 (Z-order)

| 모달 종류 | X 버튼 | ESC 키 | 외부 클릭 | 비고 |
|---|---|---|---|---|
| **S-101 첫 설정** | ❌ 없음 | ❌ 무시 | ❌ 무시 | 강제 닫기 차단 |
| **S-102 재설정** | ✅ | ✅ | ✅ | 변경 시 S-106 확인 |
| **S-103 셀 편집** | ✅ | ✅ | ✅ | 변경 시 S-106 확인 |
| **S-104 임시 폼** | - | ✅ | ❌ (모달 안) | [취소] 버튼만 |
| **S-105 새 카테고리** | ✅ | ✅ | ✅ | |
| **S-106 변경 확인** | ❌ 없음 | ✅ (= 계속편집) | ❌ 무시 | 명시적 선택 |
| **S-107 삭제 확인** | ❌ 없음 | ✅ (= 취소) | ❌ 무시 | 명시적 선택 |
| **S-108 정리 안내** | ✅ | ✅ | ❌ 무시 | |
| **S-109 손상 안내** | ❌ 없음 | ❌ 무시 | ❌ 무시 | 처리 후 자동 |
| **S-111 폴백** | ✅ | ✅ | ✅ | |
| **S-112 진단** | ✅ (= 일반 모드) | ✅ | ✅ | |
| **S-201 비고** | ✅ | ✅ | ✅ | + 모바일 스와이프 |
| **S-202 월 선택** | - | ✅ | ✅ | 외부 클릭이 일반 |
| **S-301~303 토스트** | ✅ | - | - | + 자동 닫힘 (3초) |

#### 9.3.2 중첩 모달 정책

##### 동시에 열릴 수 있는 모달
- S-103 (셀편집) + S-107 (삭제 확인)
- S-103 (셀편집) + S-104 (임시 폼)
- S-104 (임시 폼) + S-105 (새 카테고리)
...
| Base | 0 | S-001 메인 |
| Drawer | 100 | S-201 비고 |
| Modal-1 | 200 | S-103 셀 편집 |
| Modal-2 | 300 | S-104 임시 폼 |
| Modal-3 | 400 | S-105 새 카테고리 / S-107 삭제 확인 |
| Toast | 500 | S-301~303 |

##### ESC 처리 시 중첩 모달
- ESC는 가장 위 레이어부터 닫힘 (LIFO)
- 예: S-103 안에 S-107이 열린 상태 → ESC → S-107만 닫힘
...
[모달 열림]
  │
  ├─ originalState 스냅샷 저장
  │
  └─ [닫기 시도]
       │
       ├─ currentState === originalState (변경 없음)
       │     └─ 즉시 닫기
       │
       └─ currentState !== originalState (변경 있음)
             └─ S-106 확인 모달 표시
                  │
                  ├─ [닫기] ─► 변경 폐기 + 모달 닫기
                  └─ [계속 편집] ─► 모달 유지
...
##### 변경 감지 대상
- S-102 자녀 기본 설정의 모든 필드
- S-103 셀 편집의 임시 항목 목록 + 메모
- S-104 임시 항목 폼의 카테고리/이름/금액

#### 9.3.4 포커스 관리 정책

##### 모달 진입 시
- 자동 포커스: 첫 입력 필드 또는 가장 가까운 [저장] 버튼
- 포커스 트랩: Tab 키가 모달 외부로 안 빠져나감

##### 모달 종료 시
- 트리거 버튼으로 포커스 복귀
- 예: [⚙ 설정] 클릭 → S-102 → 닫음 → [⚙ 설정]에 포커스

##### 키보드 사용자 우선
- Tab 순서가 시각적 흐름과 일치
- Skip Link (선택): 헤더 → 캘린더로 점프
...
| 분기 | 레이아웃 | 캘린더 셀 높이 | 정산표 위치 |
|---|---|---|---|
| Mobile (< 768px) | 세로 스택 | 60px | 캘린더 아래 |
| Tablet (768~1023) | 세로 스택 | 80px | 캘린더 아래 |
| Desktop (1024~1439) | 세로 스택 | 90px | 캘린더 아래 |
| Wide (1440+) | 좌측 캘린더 + 우측 사이드바 | 100px | 우측 사이드바 |

##### S-103 셀 편집 모달

| 분기 | 모달 폭 | 모달 위치 |
|---|---|---|
| Mobile | 화면 95% | 화면 중앙 (또는 풀스크린) |
| Tablet+ | 480px | 화면 중앙 |
...
|---|---|
| Mobile | 바텀시트 (아래에서 위로) |
| Tablet+ | 우측 드로어 (오른쪽에서 왼쪽으로) |

##### S-202 월 선택 드롭다운

| 분기 | 형태 |
|---|---|
| Mobile | 바텀시트 (전체 12개월 스크롤) |
| Tablet+ | 드롭다운 (월 표시 영역 아래 팝오버) |

#### 9.4.3 터치 vs 마우스 인터랙션

| 인터랙션 | Mobile (Touch) | Desktop (Mouse) |
|---|---|---|
| 셀 정보 보기 | 길게 누름 (Long Press) → 툴팁 | 호버 → 툴팁 |
| 셀 편집 진입 | 짧은 탭 | 클릭 |
| 비고 닫기 | 아래로 스와이프 | X 버튼 / 외부 클릭 / ESC |
| 월 이동 | 좌우 스와이프 (선택) | ◀ ▶ 버튼 / ← → 키 |
| 임시 항목 삭제 | 좌측 스와이프 (선택) | [삭제] 버튼 |
...
##### 가로 (Landscape)
- 캘린더 7×6 그리드 (셀 폭 더 넓음)
- 정산표가 우측 사이드바로 이동
- 모달은 좌측에 좁게 표시

---

### 9.5 빈 상태 (Empty States) 정책

#### 9.5.1 빈 상태 종류

| 상태 | 위치 | 표시 내용 |
|---|---|---|
| **첫 사용 (S-101 후)** | 메인 캘린더 | 정상 캘린더 (학교 등교 자동 표시) |
| **임시 항목 0개 + 학교/학원 0** | 정산표 | "이번 달 청구할 항목이 없습니다" |
| **셀 편집 모달, 임시 항목 0개** | S-103 | "[+ 임시 항목 추가]" 버튼만 |
| **카테고리 0개 (사용자 정의)** | S-105 진입 시 | 안내 + 입력 필드 |
| **과거 달 데이터 없음** | 메인 캘린더 (과거 이동) | 빈 캘린더 (학교 자동 표시는 동작) |
...
```typescript
function shouldShowEmptyState(calc: AllowanceCalculation): boolean {
  return (
    calc.base_allowance === 0 &&
    calc.school_total === 0 &&
    calc.academy_total === 0 &&
...
### 9.6 로딩 상태 (Loading States) 정책

1단계는 로컬 동작이라 로딩이 거의 없지만, 다음 시점에는 로딩 표시:

| 시점 | 로딩 표시 | 지속 시간 |
|---|---|---|
| 앱 첫 진입 | 스플래시 또는 스켈레톤 | < 1초 |
| 월 이동 (대량 데이터 시) | 캘린더 영역 페이드 인 | < 200ms |
| 메시지 복사 | 버튼에 스피너 | < 500ms |
| 데이터 가져오기 (S-114) | 진행률 바 | 가변 |
...
### 9.7 화면 표시 우선순위 (Z-order)

```
[Layer 500: Toast / Snackbar]    ← 항상 최상위
[Layer 400: Modal-3]             ← S-105, S-107
[Layer 300: Modal-2]             ← S-104
[Layer 200: Modal-1]             ← S-103, S-102, S-101
[Layer 150: Backdrop]            ← 모달 뒤 어두운 배경
[Layer 100: Drawer / Bottom Sheet] ← S-201, S-202
[Layer 50: Tooltip]              ← S-402 호버 툴팁
[Layer 0: Main Screen]           ← S-001
```

---
## 10. 1단계 화면별 상세 명세 (Detail Specifications)

각 화면을 **개발자가 받자마자 구현 가능한 수준**으로 상세화한다.

### 10.1 S-001: 메인 캘린더 화면

#### 10.1.1 화면 목적

사용자가 매일/매월 들어와서 캘린더를 보고 임시 항목을 추가하며 청구 메시지를 복사하는 핵심 화면.
...
│  │ 종류         │ 계산식           │       합계         │ │
│  ├──────────────┼──────────────────┼────────────────────┤ │
│  │ 💰 기본 용돈 │ 80,000 × 1       │       80,000원    │ │
│  │ 🏫 학교 버스 │ 1,160 × 2 × 18   │       41,760원    │ │
│  │ 📚 학원 버스 │ 1,160 × 2 × 9    │       20,880원    │ │
│  │ 🎒 체험학습  │ 1건              │        8,000원    │ │
│  ├──────────────┴──────────────────┴────────────────────┤ │
│  │                          총 합계      150,640원        │ │
│  └──────────────────────────────────────────────────────┘ │
...
##### 영역 A: 상단 헤더 (Top Header)

| 요소 | 위치 | 동작 | 비활성 조건 |
|---|---|---|---|
| **[◀] 이전 달 버튼** | 좌측 | 클릭 → year/month -1 | 없음 (과거 무제한) |
| **월 표시 영역** | 중앙 | 클릭 → S-202 월 선택 드롭다운 | 항상 활성 |
| **[▶] 다음 달 버튼** | 좌측 | 클릭 → year/month +1 | 미래 +12개월 도달 시 |
| **[⚙ 설정] 아이콘 버튼** | 우측 | 클릭 → S-102 설정 모달 | 항상 활성 |
| **[📌 비고] 아이콘 버튼** | 우측 | 클릭 → S-201 비고 토글 | 항상 활성 |
| **[📋 메시지 복사] 아이콘 버튼** | 우측 (모바일) | 클릭 → 복사 + 토스트 | 합계 0원 시 |
...
##### 영역 B: 요일 헤더 (Weekday Header)

| 요소 | 표시 | 색상 |
|---|---|---|
| 일 (Sun) | "일" | var(--color-holiday) |
| 월 (Mon) | "월" | var(--color-text-primary) |
| 화 (Tue) | "화" | var(--color-text-primary) |
| 수 (Wed) | "수" | var(--color-text-primary) |
| 목 (Thu) | "목" | var(--color-text-primary) |
| 금 (Fri) | "금" | var(--color-text-primary) |
| 토 (Sat) | "토" | var(--color-info) |
...
.weekday-cell.weekday-sun { color: var(--color-holiday); }
.weekday-cell.weekday-sat { color: var(--color-info); }
```

##### 영역 C: 캘린더 그리드 (Calendar Grid)
...
.calendar-grid {
  display: grid;
  grid-template-columns: repeat(7, 1fr);
  grid-auto-rows: 60px; /* mobile */
  gap: 1px;
  background: var(--color-border);
}

@media (min-width: 768px) {
  .calendar-grid { grid-auto-rows: 90px; }
}
...
.calendar-cell.is-holiday .cell-number {
  color: var(--color-holiday);
}

.calendar-cell.is-weekend-sun .cell-number {
  color: var(--color-holiday);
}

.calendar-cell.is-weekend-sat .cell-number {
...
<div className="calendar-cell">
  ...
  <div className="cell-header">
    ...
    {isHoliday && (
      <span className="cell-holiday">{holidayName}</span>
    )}
  </div>

  {/* 2. 아이콘 영역 */}
  <div className="cell-icons">
    {schoolFee > 0 && <span className="cell-icon">🏫</span>}
    {academyFee > 0 && <span className="cell-icon">📚</span>}
    {hasExtraItems && <span className="cell-icon">🎒</span>}
  </div>
  ...
  {/* 4. 메모 표시기 (작은 점) */}
  {hasMemo && <div className="cell-memo-dot" />}
</div>
```
...
| 상황 | Hover (Desktop) | Click | Long Press (Mobile) |
|---|---|---|---|
| 일반 셀 (값 있음) | 배경 강조 + 툴팁 | S-103 셀 편집 모달 | 툴팁 표시 |
| 빈 셀 (placeholder) | - | 동작 없음 | - |
| 공휴일 + 값 있음 | 동일 | S-103 | 동일 |
| 공휴일 + 값 없음 | 배경 강조 (선택 가능) | S-103 (메모 추가용) | - |
| 오늘 셀 | 강조 테두리 + 툴팁 | S-103 | 동일 |
...
##### 영역 D: 정산표 (Summary Table)

###### 정산표 구조

```jsx
<section className="summary-section">
  <h2 className="summary-title">{`${year}년 ${month}월 정산`}</h2>
  ...
  <div className="summary-table">
    ...
    {baseAllowance > 0 && (
      <SummaryRow
        icon="💰"
        label="기본 용돈"
        formula={`${formatNumber(baseAllowance)} × 1`}
    ...
    {schoolTotal > 0 && (
      <SummaryRow
        icon="🏫"
        label="학교 버스"
        formula={`${formatNumber(schoolFare)} × ${schoolMultiplier} × ${schoolDays}일`}
        amount={schoolTotal}
      />
    )}

    {/* 학원 버스 행 */}
    {academyTotal > 0 && <SummaryRow ... />}
  ...
  <div className="summary-total">
    <span>총 합계</span>
    <strong>{formatCurrency(total)}원</strong>
  </div>
</section>
```
...
.summary-row-icon { font-size: 20px; }
.summary-row-formula {
  font-size: var(--font-size-sm);
  color: var(--color-text-secondary);
}
...
| 요소 | 동작 |
|---|---|
| 행 클릭 (학교/학원) | 동작 없음 (정보 표시만) |
| 행 클릭 (임시 항목) | 해당 카테고리 첫 항목의 셀로 스크롤 (선택 기능) |
| 합계 행 | 동작 없음 |
...
[페이지 첫 진입]
  │
  ├─ 1. localStorage 검사
  │     ├─ settings_v1 없음 ─► S-101 (강제)
  │     └─ settings_v1 있음 ─► 다음 단계
  │
  ├─ 2. meta_v1 로드 → current_view_month 확인
  │     ├─ 있음 ─► 마지막 본 달
  │     └─ 없음 ─► 오늘이 속한 달
  │
  ├─ 3. holidays.json fetch (캐시 우선)
  │
  ├─ 4. settings_v1 로드
  │
  ├─ 5. calendar_v1_YYYY_MM 로드
  │     └─ 없음 ─► 빈 객체 사용
  │
  ├─ 6. custom_categories_v1 로드
  │
  ├─ 7. 캘린더 계산 (calculator.js)
  │     └─ 각 셀의 school_fee, academy_fee, extra_total 계산
  │
  ├─ 8. 정산표 계산
  │
  └─ 9. 렌더링
...
interface MainScreenState {
  ...
  // 데이터
  settings: Settings_v1 | null;
  calendar: CalendarMonth_v1 | null;
  customCategories: CustomCategory[];
  holidays: Holidays;

  // 계산 결과 (캐시)
  calculation: AllowanceCalculation | null;

  // UI 상태
  selectedCellDate: string | null;     // 셀 편집 모달 열렸을 때
  showSettings: boolean;               // 설정 모달
  showNotes: boolean;                  // 비고 드로어
  showMonthSelector: boolean;          // 월 선택
  showWelcome: boolean;                // 첫 사용 모달
...
#### 10.1.6 인수 조건 (Acceptance Criteria)

```gherkin
Given 사용자가 메인 화면에 진입하고 settings_v1, calendar_v1_2026_05가 저장되어 있을 때
When 화면이 로드되면
Then 1초 이내에 캘린더가 렌더링되어야 한다
And 헤더에 "2026년 5월"이 표시되어야 한다
And 5월의 모든 평일 셀에 학교 버스 표시가 되어야 한다
And 5/14 (체험학습 등록된 날) 셀에 🎒 아이콘이 표시되어야 한다
And 정산표에 4개 행 + 합계가 표시되어야 한다
And 하단 액션바에 [📋 메시지 복사] 버튼이 활성 상태로 표시되어야 한다

Given 메인 화면이 표시된 상태
When 사용자가 화면 폭을 768px 미만으로 줄이면
Then 헤더가 모바일 레이아웃으로 전환되어야 한다 (아이콘 버튼만 표시)
And 캘린더 셀 높이가 60px로 변경되어야 한다
And 정산표가 캘린더 아래로 이동해야 한다

Given 메인 화면이 표시된 상태
When 사용자가 [▶ 다음 달] 버튼을 클릭하면
Then 200ms 이내에 다음 달 캘린더가 표시되어야 한다
And 헤더의 월 표시가 갱신되어야 한다
And 정산표가 새 달 기준으로 재계산되어야 한다

Given 메인 화면 + 빈 5월 (학교/학원/임시 모두 없음)
When 정산표를 보면
Then "이번 달 청구할 항목이 없습니다" 빈 상태 메시지가 표시되어야 한다
And [📋 메시지 복사] 버튼이 비활성 상태여야 한다
```
...
특정 날짜 셀의 임시 항목을 추가/수정/삭제하고 메모를 입력하는 모달.

#### 10.2.2 모달 영역 구조

```
┌─ 5월 14일 (수) [어린이날] ───────[×]┐ ← 모달 헤더 (60px)
├──────────────────────────────────────┤
│  📋 기본 항목 (자동, 수정 불가)      │ ← 섹션 1
│  ┌──────────────────────────────┐   │
│  │ 🏫 학교 등교        2,320원   │   │
...
| 요소 | 내용 | 디자인 |
|---|---|---|
| 날짜 표시 | "5월 14일 (수)" | 16px bold |
| 공휴일 배지 | "[어린이날]" (있을 때만) | 빨간 배경 + 흰 글씨 12px |
| [×] 닫기 버튼 | 우측 상단 | 24×24 아이콘 |
...
<ModalHeader>
  <div className="modal-date">
    ...
    {isHoliday && (
      <Badge variant="holiday">{holidayName}</Badge>
    )}
  </div>
...
```jsx
{(schoolFee > 0 || academyFee > 0) && (
  <Section title="기본 항목" subtitle="자동, 수정 불가">
    {schoolFee > 0 && (
      <ReadOnlyRow icon="🏫" label="학교 등교" amount={schoolFee} />
    )}
    {academyFee > 0 && (
      <ReadOnlyRow icon="📚" label="학원 등원" amount={academyFee} />
    )}
  </Section>
)}
```
...
.extra-item-actions button:hover {
  background: var(--color-bg);
}
```
...
<div className="extra-item-card">
  <div className="extra-item-info">
    <div className="extra-item-name">
      {getCategoryIcon(item.category)} {item.name}
    </div>
    <div className="extra-item-amount">
      {formatCurrency(item.amount)}원
    </div>
  </div>
  <div className="extra-item-actions">
    <button onClick={() => onEdit(item.id)} aria-label="수정">
      ✏️
    </button>
...
| 상태 | 표시 | 동작 |
|---|---|---|
| 항목 0~2개 | "[+ 임시 항목 추가]" 활성 | 클릭 → S-104 폼 열림 |
| 항목 3개 (한도) | 버튼 비활성 + "최대 3개" 안내 | 클릭 안 됨 |
...
```jsx
<Section title="메모" subtitle="200자 이내">
  <TextArea
    value={memo}
    onChange={handleMemoChange}
    maxLength={200}
  ...
  <div className={cn(
    "char-counter",
    memo.length > 180 && "char-counter--warning",
    memo.length === 200 && "char-counter--limit"
  )}>
    {memo.length}/200
  </div>
</Section>
...
| 글자 수 | 색상 |
|---|---|
| 0~180 | 회색 (var(--color-text-secondary)) |
| 181~199 | 주황 (var(--color-warning)) |
| 200 | 빨강 (var(--color-error)) |

##### 영역 E: 푸터

| 버튼 | 위치 | 동작 |
|---|---|---|
| [취소] | 우측 첫 번째 | 변경 있으면 S-106, 없으면 즉시 닫기 |
| [저장] | 우측 두 번째 | 검증 → 저장 → 토스트 → 닫기 |

#### 10.2.4 모달 상태 관리

```typescript
interface CellEditModalState {
  // 원본 데이터 (변경 감지용)
  original: {
  ...
  // 하위 모달
  showAddForm: boolean;            // S-104 임시 항목 폼
  editingItemId: string | null;    // 수정 중인 항목 ID
  showDeleteConfirm: string | null; // 삭제 확인 대상 ID

  // UI
  validationErrors: Record<string, string>;
}

function isDirty(state: CellEditModalState): boolean {
...
```
1. 사용자가 캘린더 셀 클릭
2. 시스템: cell.date 상태 저장
3. 시스템: calendar_v1의 해당 cell 데이터 로드
   - extra_items, memo 추출
   - 없으면 빈 배열/빈 문자열
4. 시스템: original = 로드된 데이터 스냅샷
5. 시스템: draft = original 복사본
6. 시스템: S-103 모달 표시 + 슬라이드 인 애니메이션
7. 시스템: 자동 포커스 → 첫 임시 항목 [수정] 또는 [+ 추가] 또는 메모
```
...
```
1. [+ 임시 항목 추가] 클릭
2. S-104 폼 모달 표시 (z-index 300)
3. 사용자 입력 + [추가] 클릭
4. 검증 통과 시:
   - draft.extraItems.push(newItem)
   - S-104 닫기
5. 검증 실패 시:
   - S-104에 에러 메시지 표시 (모달 닫지 않음)
```
...
```
1. [✏️ 수정] 아이콘 클릭
2. editingItemId = item.id 설정
3. S-104 폼 모달 표시 (편집 모드, prefilled)
4. 사용자 수정 + [저장] 클릭
5. 검증 통과 시:
   - draft.extraItems의 해당 항목 업데이트
   - S-104 닫기
```
...
```
1. [🗑] 아이콘 클릭
2. showDeleteConfirm = item.id 설정
3. S-107 삭제 확인 모달 표시 (z-index 400)
4. [삭제] 클릭 시:
   - draft.extraItems에서 해당 항목 제거
   - S-107 닫기
5. [취소] 클릭 시:
...
3. 검증 실패: 에러 표시, 모달 유지
4. 검증 통과:
   - draft → calendar_v1_YYYY_MM 저장
   - localStorage 쓰기
5. 저장 성공:
   - S-103 닫기
   - S-001 메인 화면 캘린더 갱신 (해당 셀)
   - S-301 토스트 "저장되었습니다"
6. 저장 실패 (스토리지 가득):
   - S-108 정리 안내 모달 표시
```
...
```
1. [취소] 또는 [×] 또는 ESC 또는 외부 클릭
2. 시스템: isDirty(state) 검사
3. dirty == false:
   - 즉시 모달 닫기
4. dirty == true:
   - S-106 변경 확인 모달 표시
   - [닫기] → 변경 폐기 + S-103 닫기
   - [계속 편집] → S-106만 닫기, S-103 유지
```
...
```gherkin
Given 메인 화면이 표시된 상태
When 사용자가 5/14 캘린더 셀을 클릭하면
Then 셀 편집 모달이 표시되어야 한다
And 모달 헤더에 "5월 14일 (수)"가 표시되어야 한다
And 5/14가 공휴일이면 공휴일 이름 배지가 표시되어야 한다
And 학교/학원 자동 항목이 읽기 전용으로 표시되어야 한다
And 임시 항목 섹션이 표시되어야 한다
And 메모 텍스트 영역이 표시되어야 한다

Given 셀 편집 모달이 열린 상태 + 임시 항목 0개
When [+ 임시 항목 추가] 버튼을 클릭하면
Then S-104 임시 항목 폼이 모달 위에 표시되어야 한다
And 카테고리 드롭다운에 자동 포커스되어야 한다

Given 셀 편집 모달 + 임시 항목 3개
When 화면을 보면
Then [+ 임시 항목 추가] 버튼이 비활성화되어야 한다
And "최대 3개까지 추가할 수 있습니다" 안내가 표시되어야 한다

Given 셀 편집 모달 + 임시 항목 1개 + 메모 변경한 상태
When [×] 또는 [취소] 버튼을 클릭하면
Then S-106 변경 사항 확인 모달이 표시되어야 한다
And [닫기] 클릭 시 변경 사항이 폐기되어야 한다
And [계속 편집] 클릭 시 S-103이 유지되어야 한다

Given 셀 편집 모달에서 메모를 200자까지 입력
When 추가 글자를 입력하면
Then 입력이 차단되어야 한다
And "200/200" 카운터가 빨간색으로 표시되어야 한다
```
...
### 10.3 S-104: 임시 항목 추가/수정 폼

#### 10.3.1 화면 목적

임시 항목 1건을 추가하거나 기존 항목을 수정하는 폼 모달.
...
```
1. 사용자가 [+ 새 카테고리] 클릭
2. S-105 새 카테고리 모달 표시
3. 사용자 입력 + [추가]
4. custom_categories_v1에 저장
5. S-105 닫기
6. 드롭다운에서 새로 추가한 카테고리 자동 선택
```
...
function CurrencyInput({ value, onChange, min = 0, max = 10000000 }: CurrencyInputProps) {
  // 화면 표시값 (천 단위 콤마)
...
| "8000" | 8000 | "8,000" |
| "8,000" | 8000 | "8,000" |
| "8a000" | 8000 | "8,000" (a 무시) |
| "" | 0 | "" |
| "-100" | 0 | "" (음수 무시) |
| "10000001" | 10000000 | "10,000,000" (max) |
...
|---|---|---|
| 카테고리 | 빈 값 X | 제출 시 |
| 이름 | 1~50자 | 제출 시 (실시간 글자수 표시) |
| 금액 | 1 이상 | 제출 시 |

```typescript
function validate(input: ExtraItemInput): ValidationResult {
...
```
1. 진입 시: 빈 폼 (카테고리 미선택, 이름 빈값, 금액 0)
2. 자동 포커스: 카테고리 드롭다운
3. 사용자 입력
4. [추가] 클릭
5. 검증 통과:
   - 신규 ID 생성 (ex_ + nanoid(6))
   - parent S-103의 draft.extraItems에 push
   - S-104 닫기
6. 검증 실패:
   - 필드별 에러 메시지 표시
   - 모달 유지
...
```
1. 진입 시: 기존 항목 데이터로 prefilled
2. 자동 포커스: 이름 필드 (가장 자주 수정)
3. 사용자 수정
4. [저장] 클릭
5. 검증 통과:
   - parent S-103의 draft.extraItems에서 해당 ID 항목 업데이트
   - S-104 닫기
6. 검증 실패: 동일
```
...
```gherkin
Given S-103 셀 편집 모달이 열린 상태
When 사용자가 [+ 임시 항목 추가] 버튼을 클릭하면
Then S-104 폼 모달이 표시되어야 한다
And 카테고리 드롭다운에 자동 포커스되어야 한다
And 모든 필드가 빈 상태여야 한다
And 모달 제목이 "임시 항목 추가"로 표시되어야 한다

Given S-104 폼이 열린 상태
When 사용자가 카테고리 미선택으로 [추가] 버튼을 클릭하면
Then "카테고리를 선택해주세요" 에러가 카테고리 필드 아래 표시되어야 한다
And 모달은 닫히지 않아야 한다

Given S-104 폼 + 카테고리 "체험학습", 이름 "박물관", 금액 8000 입력 상태
When [추가] 버튼을 클릭하면
Then S-104 폼이 닫혀야 한다
And S-103 셀 편집 모달의 임시 항목 목록에 새 항목이 표시되어야 한다
And 항목 카드에 "🎒 박물관 / 8,000원"이 표시되어야 한다

Given S-104 폼이 열린 상태
When 사용자가 카테고리 드롭다운에서 [+ 새 카테고리]를 선택하면
Then S-105 새 카테고리 모달이 표시되어야 한다
And [추가] 시 S-105가 닫히고 새 카테고리가 자동 선택되어야 한다

Given S-104 수정 모드 (기존 항목 편집)
When 모달이 열리면
Then 기존 데이터로 모든 필드가 채워져야 한다
And 모달 제목이 "임시 항목 수정"으로 표시되어야 한다
And [추가] 대신 [저장] 버튼이 표시되어야 한다
```

---
### 10.4 S-101 / S-102: 자녀 기본 설정 모달

S-101 (첫 사용)과 S-102 (재설정)는 **동일한 폼 구조**를 사용하지만, 진입 조건과 닫기 정책이 다르다.

#### 10.4.1 모달 차이점

| 항목 | S-101 (첫 사용) | S-102 (재설정) |
|---|---|---|
| 진입 조건 | settings_v1 없음 | [⚙ 설정] 버튼 클릭 |
| 닫기 버튼 | ❌ 없음 | ✅ 있음 |
| ESC 키 | ❌ 무시 | ✅ 닫기 |
| 외부 클릭 | ❌ 무시 | ✅ 닫기 |
| 모달 제목 | "처음 시작합니다 👋" | "자녀 정보 변경" |
| 부제목 | "자녀 정보를 한 번만 등록해주세요" | "변경 사항은 즉시 반영됩니다" |
| 저장 버튼 | "저장하고 시작하기" | "저장" |
| 취소 버튼 | ❌ 없음 | "취소" |
| 변경 후 닫기 | (강제 차단) | S-106 확인 |
...
// 동작
- maxLength: 20
- placeholder: "예: 자녀A (생략 가능)"
- 자동 trim 처리 (저장 시)
- 빈 값 허용
```

##### B. 기본 용돈 (CurrencyInput)
...
.weekday-button.weekend-sun {
  color: var(--color-holiday);
}

.weekday-button.weekend-sun.selected {
...
<RadioGroup
  ...
  options={[
    { value: false, label: "편도", icon: "→" },
    { value: true, label: "왕복", icon: "↔" }
  ]}
/>
```
...
<RadioGroup
  name="school.holiday_attend"
  value={attendOnHoliday}
  onChange={setAttendOnHoliday}
  options={[
...
<Section title="📚 학원 (선택)">
  ...
  {/* 학원 등원 요일 0개일 때 안내 */}
  {academyDays.length === 0 && (
    <Hint>학원 등원 요일을 선택하면 단가를 입력할 수 있습니다</Hint>
  )}
  ...
  <FormRow>
    <CurrencyInput
      label="버스비 단가"
      value={academyFare}
      onChange={setAcademyFare}
      disabled={academyDays.length === 0}
    />
    <RadioGroup
      label="편도/왕복"
      disabled={academyDays.length === 0}
...
interface SettingsFormState {
  ...
  touched: Record<string, boolean>;  // 필드별 입력 시도 여부
}
```
...
| 시점 | 검증 대상 |
|---|---|
| 입력 중 (onChange) | 글자 수, 숫자 범위 (소프트 안내) |
| 필드 떠날 때 (onBlur) | 해당 필드만 (touched=true 처리) |
| [저장] 클릭 | 모든 필드 |

##### 검증 함수

```typescript
function validateSettings(form: SettingsFormState): ValidationResult {
  const errors: Record<string, string> = {};

  // 자녀 이름
  if (form.child_name.length > 20) {
    errors.child_name = "20자 이내로 입력해주세요";
  }

  // 기본 용돈
  if (form.base_allowance < 0) {
    errors.base_allowance = "0원 이상 입력해주세요";
  ...
  if (form.school.days.length > 0) {
    if (!form.school.fare || form.school.fare < 1) {
      errors["school.fare"] = "학교 단가를 입력해주세요";
  ...
  if (form.academy.days.length > 0) {
    if (!form.academy.fare || form.academy.fare < 1) {
      errors["academy.fare"] = "학원 단가를 입력해주세요";
...
```gherkin
Given settings_v1이 없는 첫 사용자
When 앱에 진입하면
Then S-101 환영 모달이 자동으로 표시되어야 한다
And 모달의 [×] 닫기 버튼이 없어야 한다
And ESC 키를 눌러도 닫히지 않아야 한다
And 모달 외부를 클릭해도 닫히지 않아야 한다

Given S-101 모달이 열린 상태
When 사용자가 모든 필수 필드를 정상 입력하고 [저장하고 시작하기] 버튼을 클릭하면
Then settings_v1이 localStorage에 저장되어야 한다
And meta_v1이 localStorage에 생성되어야 한다
And S-101이 닫혀야 한다
And S-001 메인 캘린더 화면이 표시되어야 한다

Given S-102 재설정 모달이 열린 상태
When 사용자가 학교 등교 요일을 변경하고 [취소] 버튼을 클릭하면
Then S-106 변경 사항 확인 모달이 표시되어야 한다

Given S-102에서 학원 등원 요일을 모두 미체크 상태
When 학원 단가 필드를 보면
Then 입력 필드가 비활성(회색) 상태여야 한다
And 학원 편도/왕복 라디오도 비활성이어야 한다
And 학원 공휴일 처리 라디오도 비활성이어야 한다

Given S-102에서 학원 등원 요일을 1개 이상 체크
When 화면을 보면
Then 학원 단가/편도/왕복/공휴일 필드가 모두 활성화되어야 한다
And 학원 단가가 비어있으면 [저장] 시 검증 실패해야 한다
```

---

### 10.5 S-201: 비고 드로어 / 바텀시트

#### 10.5.1 화면 목적

아이콘 의미와 한국 시내버스 요금표 등의 참고 정보 표시.
...
<Section title="아이콘 안내">
  <IconList>
    {[
      { icon: "🏫", label: "학교 등교 (왕복 버스)" },
      { icon: "📚", label: "학원 등원 (왕복 버스)" },
      { icon: "🎒", label: "임시 항목 (체험학습 등)" },
      { icon: "🔴", label: "법정공휴일 (등교일은 빨간색)" }
    ].map(item => (
      <IconRow key={item.icon}>
...
```jsx
<Section title="경기도 시내버스 일반형 요금">
  <FareTable>
    <thead>
      <tr>
        <th>대상</th>
        <th align="right">편도</th>
        <th align="right">왕복</th>
      </tr>
    </thead>
    <tbody>
      {KOREAN_BUS_FARES.fares.map(fare => (
        <tr key={fare.category} className={cn(fare.highlighted && "highlighted")}>
          <td>
            {fare.category}
            {fare.highlighted && <span className="badge">★</span>}
            <small>{fare.age_range}</small>
          </td>
          <td align="right">{formatCurrency(fare.one_way)}원</td>
          <td align="right">{formatCurrency(fare.round_trip)}원</td>
        </tr>
      ))}
    </tbody>
  </FareTable>
  ...
  <SourceInfo>
    <div>출처: 경기버스운송사업조합 (gbus.or.kr)</div>
    <div>시행일: 2025-10-25</div>
  </SourceInfo>
</Section>
```
...
function useSwipeToClose(onClose: () => void) {
  ...
  const handleTouchMove = (e: TouchEvent) => {
    ...
    if (delta > 0) setCurrentY(delta);
  };

  const handleTouchEnd = () => {
    if (currentY > 100) onClose();  // 100px 이상 스와이프 시 닫기
    setCurrentY(0);
    setStartY(null);
  };

  return { handleTouchStart, handleTouchMove, handleTouchEnd, currentY };
}
```
...
```gherkin
Given 메인 화면 (모바일, 폭 < 768px)
When 사용자가 [📌 비고] 버튼을 클릭하면
Then S-201 바텀시트가 화면 하단에서 슬라이드 업되어야 한다
And 메인 화면 위에 어두운 백드롭이 표시되어야 한다
And 바텀시트 상단에 드래그 핸들이 표시되어야 한다

Given 메인 화면 (데스크톱, 폭 ≥ 768px)
When 사용자가 [📌 비고] 버튼을 클릭하면
Then S-201 드로어가 화면 우측에서 슬라이드 인되어야 한다
And 폭이 320px여야 한다

Given S-201이 열린 상태 (모바일)
When 사용자가 바텀시트를 아래로 100px 이상 스와이프하면
Then 바텀시트가 닫혀야 한다
...
Given S-201이 표시된 상태
When 사용자가 요금표를 보면
Then 청소년 행이 강조 표시되어야 한다 (★ 표시 + 배경색)
And 출처와 시행일이 하단에 표시되어야 한다
```

---

### 10.6 S-202: 월 선택 드롭다운

#### 10.6.1 화면 목적

±12개월 범위에서 원하는 월로 빠르게 이동.
...
>
  <YearTabs>
    <YearTab
      year={currentYear - 1}
      isActive={selectedYear === currentYear - 1}
      onClick={() => setSelectedYear(currentYear - 1)}
    >
      {currentYear - 1}년
    </YearTab>
    <YearTab year={currentYear} ...>★ {currentYear}년</YearTab>
    <YearTab year={currentYear + 1} ...>{currentYear + 1}년</YearTab>
  </YearTabs>

  <MonthGrid>
    {Array.from({length: 12}, (_, i) => i + 1).map(month => {
      const isCurrentMonth = selectedYear === currentYear && month === currentMonthNum;
      const isFutureLimit = isMonthBeyondLimit(selectedYear, month);
      return (
        <MonthCell
          key={month}
          month={month}
...
```typescript
function isMonthBeyondLimit(year: number, month: number): boolean {
  const today = new Date();
...
```gherkin
Given 메인 화면이 표시된 상태
When 사용자가 헤더의 월 표시 영역 ("2026년 5월")을 클릭하면
Then 데스크톱에서는 팝오버가 월 표시 아래에 나타나야 한다
And 모바일에서는 바텀시트가 화면 하단에서 슬라이드 업되어야 한다

Given 월 선택 드롭다운이 열린 상태
When 5월 셀을 보면
Then 5월에 ★ 표시가 있고 강조 색상으로 표시되어야 한다 (현재 월)

Given 월 선택 드롭다운이 열린 상태
When 사용자가 "8월" 셀을 클릭하면
Then 메인 캘린더가 2026년 8월로 변경되어야 한다
And 드롭다운이 닫혀야 한다

Given 오늘이 2026년 5월
When 월 선택 드롭다운에서 2027년 6월을 보면
Then 비활성 상태여야 한다 (12개월 초과)
And 클릭해도 동작하지 않아야 한다
```

---

### 10.7 S-105: 사용자 정의 카테고리 추가 모달

#### 10.7.1 화면 목적

기본 카테고리(9개) 외에 사용자가 자주 쓰는 카테고리를 추가.
...
```jsx
{/* 1단계 초기에는 미포함, 이모지 직접 입력은 v1.1로 */}
<input
  type="text"
  placeholder="직접 입력"
  maxLength={2}  /* 이모지 1자 (서로게이트 페어로 2자) */
  value={customEmoji}
...
```typescript
function validateCustomCategory(input: { name: string; icon: string }) {
  const errors: Record<string, string> = {};
  ...
  if (existing.some(c => c.name === input.name.trim())) {
...
```typescript
async function addCustomCategory(input: { name: string; icon: string }): Promise<Result> {
  const validation = validateCustomCategory(input);
  if (!validation.valid) return { success: false, errors: validation.errors };
  ...
  return saveCustomCategories(existing);
}
```
...
```gherkin
Given S-104 임시 항목 폼이 열린 상태
When 사용자가 카테고리 드롭다운에서 [+ 새 카테고리]를 선택하면
Then S-105 새 카테고리 모달이 표시되어야 한다
And 자주 쓰이는 이모지 그리드가 표시되어야 한다

Given S-105 모달 + 이름 "동아리 회비" 입력 + 🎵 아이콘 선택
When 사용자가 [추가] 버튼을 클릭하면
Then 새 카테고리가 custom_categories_v1에 저장되어야 한다
And S-105가 닫혀야 한다
And S-104의 카테고리 드롭다운에서 "🎵 동아리 회비"가 자동 선택되어야 한다

Given S-105에서 이름 "체험학습" 입력 (기본 카테고리와 동일)
When [추가] 버튼을 클릭하면
Then "이미 존재하는 카테고리입니다" 에러가 표시되어야 한다
```

---
### 10.8 S-106: 변경 사항 확인 모달

#### 10.8.1 화면 목적

저장 안 한 변경 사항이 있는 상태에서 모달을 닫으려 할 때 확인.
...
| 액션 | 결과 |
|---|---|
| [계속 편집] (Primary) | S-106 닫기, 원래 모달 유지 |
| [닫기] (Destructive) | 변경 폐기, S-106 + 원래 모달 모두 닫기 |
| ESC | [계속 편집]과 동일 (안전한 방향) |
| 외부 클릭 | 무시 (강제 선택 유도) |
...
.confirm-actions button[data-variant="destructive"] {
...
```gherkin
Given S-103 셀 편집 모달에 변경 사항이 있는 상태
When 사용자가 [취소] 버튼을 클릭하면
Then S-106 변경 사항 확인 모달이 표시되어야 한다
And [계속 편집]과 [닫기] 버튼이 표시되어야 한다

Given S-106이 표시된 상태
When 사용자가 [계속 편집] 버튼을 클릭하면
Then S-106만 닫히고 S-103은 유지되어야 한다

Given S-106이 표시된 상태
When 사용자가 [닫기] 버튼을 클릭하면
Then S-106과 S-103이 모두 닫히고
And 변경 사항이 폐기되어야 한다 (저장 안 됨)

Given S-106이 표시된 상태
When 사용자가 ESC 키를 누르면
Then [계속 편집]과 동일하게 동작해야 한다 (안전한 방향)
And 외부 클릭 시에는 동작하지 않아야 한다 (명시적 선택 유도)
```

---

### 10.9 S-107: 임시 항목 삭제 확인 모달
...
| 액션 | 결과 |
|---|---|
| [취소] | S-107 닫기, S-103 유지 |
| [삭제] (Destructive) | draft.extraItems에서 제거, S-107 닫기 |
| ESC | [취소]와 동일 |
| 외부 클릭 | 무시 |
...
```gherkin
Given S-103 셀 편집 모달에 임시 항목 1개 표시
When 사용자가 [🗑] 삭제 아이콘을 클릭하면
Then S-107 삭제 확인 모달이 표시되어야 한다
And 삭제할 항목의 이름과 금액이 표시되어야 한다

Given S-107이 표시된 상태
When 사용자가 [삭제] 버튼을 클릭하면
Then S-107이 닫히고 S-103의 임시 항목 목록에서 해당 항목이 제거되어야 한다
But localStorage에는 아직 저장되지 않아야 한다 (S-103 [저장] 시 저장)

Given S-107이 표시된 상태
When 사용자가 [취소] 버튼을 클릭하면
Then S-107만 닫히고 S-103의 항목 목록은 변경되지 않아야 한다
```

---

### 10.10 S-108: 데이터 정리 안내 모달

#### 10.10.1 화면 목적

로컬스토리지가 가득 차서 저장 실패 시 오래된 데이터 정리 안내.
...
| 액션 | 결과 |
|---|---|
| [내보내기 후 정리] | S-113 내보내기 → 후 정리 |
| [지금 정리] (Destructive) | 6개월 이전 calendar_v1_* 삭제 |
| [취소] | 모달 닫기, 사용자가 다른 방법 강구 (브라우저 캐시 정리 등) |

#### 10.10.4 정리 로직

```typescript
function cleanupOldCalendars(retainMonths: number = 6): CleanupResult {
  const now = new Date();
  const cutoff = new Date(now.getFullYear(), now.getMonth() - retainMonths, 1);
  const cutoffKey = `calendar_v1_${cutoff.getFullYear()}_${pad(cutoff.getMonth() + 1)}`;

  const allKeys = Object.keys(localStorage);
  const toDelete: string[] = [];

  allKeys.forEach(key => {
    if (key.startsWith("calendar_v1_") && key < cutoffKey) {
      toDelete.push(key);
    }
  });
  ...
  return {
    ...
    freedBytes: estimateFreed(toDelete)
  };
}
```
...
```gherkin
Given 로컬스토리지가 가득 찬 상태에서 사용자가 새 임시 항목 추가 시도
When 저장이 실패하면
Then S-108 데이터 정리 안내 모달이 자동으로 표시되어야 한다
And 정리될 캘린더 목록이 표시되어야 한다 (6개월 이전)

Given S-108 모달이 표시된 상태
When 사용자가 [지금 정리] 버튼을 클릭하면
Then 6개월 이전 calendar_v1_* 키들이 삭제되어야 한다
And S-108이 닫혀야 한다
And 원래 시도했던 작업이 자동으로 재시도되어야 한다

Given S-108 모달이 표시된 상태
When 사용자가 [내보내기 후 정리] 버튼을 클릭하면
Then S-113 데이터 내보내기 모달이 먼저 표시되어야 한다
And 내보내기 완료 후 정리가 진행되어야 한다
```

---

### 10.11 S-109: 데이터 손상 안내 모달

#### 10.11.1 화면 목적

settings_v1 또는 핵심 데이터의 JSON 파싱 실패 시 안내 + 복구 절차.
...
│  손상된 데이터는 자동으로 백업    │
│  되었습니다.                      │
│  (settings_v1_corrupted_TIMESTAMP)│
│                                  │
│  [개발자 도구로 백업 확인]         │
│                                  │
│  [다시 설정하기]                  │
...
| 액션 | 결과 |
|---|---|
| [다시 설정하기] | S-109 닫기 → S-101 강제 표시 |
| [개발자 도구로 백업 확인] | 콘솔에 백업 키 출력 |

#### 10.11.4 손상 감지 로직

```typescript
function detectAndBackupCorruption(key: string): boolean {
  ...
  try {
    ...
    // 스키마 검증
    if (key === "settings_v1") {
      if (!data.school || !data.academy || typeof data.base_allowance !== "number") {
        backup(key);
        return true;
      }
    }
...
function backup(key: string): void {
  const value = localStorage.getItem(key);
  if (!value) return;
  const backupKey = `${key}_corrupted_${Date.now()}`;
  localStorage.setItem(backupKey, value);
...
### 10.12 S-111: 클립보드 폴백 모달

#### 10.12.1 화면 목적

`navigator.clipboard.writeText()` 미지원 또는 권한 거부 시 수동 복사 안내.
...
```jsx
<FallbackCopyModal isOpen={showFallback} message={generatedMessage}>
  <textarea
    ref={textareaRef}
    readOnly
    value={message}
    rows={15}
    onFocus={(e) => e.target.select()}  /* 포커스 시 자동 선택 */
  />
  <Button onClick={() => textareaRef.current?.select()}>
    모두 선택
  </Button>
</FallbackCopyModal>
```
...
```gherkin
Given 구형 브라우저 (clipboard API 미지원)에서 메인 화면
When 사용자가 [📋 메시지 복사] 버튼을 클릭하면
Then S-111 폴백 모달이 표시되어야 한다
And 메시지가 textarea에 표시되어야 한다
And textarea에 자동 포커스 + 텍스트 자동 선택되어야 한다

Given S-111이 표시된 상태
When 사용자가 [모두 선택] 버튼을 클릭하면
Then textarea의 모든 텍스트가 선택되어야 한다 (사용자가 Ctrl+C 가능)
```

---

### 10.13 S-301~303: 토스트 / 스낵바

#### 10.13.1 토스트 종류

| ID | 종류 | 색상 | 자동 닫힘 | 사용 사례 |
|---|---|---|---|---|
| S-301 | Success | 녹색 | 3초 | 저장 성공, 복사 성공 |
| S-302 | Error | 빨강 | 5초 | 저장 실패, 검증 실패 |
| S-303 | Warning | 주황 | 4초 | 한도 도달, 임시 경고 |
| (S-304) | Info | 파랑 | 3초 | 일반 안내 (사용 안 함) |

#### 10.13.2 토스트 영역 구조

```
       ┌──────────────────────────────────┐
       │ ✅  메시지가 복사되었습니다        │  ← S-301 Success
       └──────────────────────────────────┘
                                            (3초 후 자동 닫힘)

       ┌──────────────────────────────────┐
       │ ❌  저장에 실패했습니다           │  ← S-302 Error
       │     [다시 시도]                  │
       └──────────────────────────────────┘
                                            (5초 또는 액션)

       ┌──────────────────────────────────┐
       │ ⚠️  최대 3개까지 추가할 수 있습니다│  ← S-303 Warning
       └──────────────────────────────────┘
                                            (4초)
```
...
.toast-action button { font-weight: var(--font-weight-medium); }
.toast-close { padding: 4px; cursor: pointer; }
...
function useToast() {
  ...
  function showToast(opts: ToastOptions) {
    const id = nanoid();
    const toast = { id, ...opts, duration: opts.duration ?? getDefaultDuration(opts.type) };
...
#### 10.13.6 토스트 표시 사례 매트릭스

| 사용 시점 | 종류 | 메시지 | 지속 |
|---|---|---|---|
| 메시지 복사 성공 | Success | "📋 복사되었습니다" | 3초 |
| 자녀 설정 저장 성공 | Success | "✅ 저장되었습니다" | 3초 |
| 셀 편집 저장 성공 | Success | "✅ 저장되었습니다" | 3초 |
| 임시 항목 추가 | Success | "✅ 추가되었습니다" | 3초 |
| 임시 항목 삭제 | Success | "🗑 삭제되었습니다" | 3초 |
| 사용자 정의 카테고리 추가 | Success | "✅ 카테고리 추가됨" | 3초 |
| 검증 실패 (필드별) | (인라인 처리, 토스트 X) | - | - |
| 저장 실패 (스토리지) | Error + Action | "저장 실패. [다시 시도]" | 0 (수동) |
| 클립보드 권한 거부 | Error | "복사 실패. 모달에서 수동 복사하세요" | 5초 |
| 임시 항목 한도 도달 | Warning | "최대 3개까지 추가할 수 있습니다" | 4초 |
| 미래 +12개월 도달 | Warning | "12개월 후까지만 표시됩니다" | 4초 |
| 메모 글자수 도달 | (인라인 카운터, 토스트 X) | - | - |

#### 10.13.7 인수 조건

```gherkin
Given 메인 화면
When 사용자가 [📋 메시지 복사] 버튼을 클릭하고 정상 복사되면
Then S-301 Success 토스트가 표시되어야 한다
And "📋 복사되었습니다" 메시지가 표시되어야 한다
And 3초 후 자동으로 사라져야 한다

Given 토스트가 표시된 상태
When 사용자가 토스트의 [×] 버튼을 클릭하면
Then 토스트가 즉시 사라져야 한다

Given 저장 실패 상황
When S-302 Error 토스트가 표시되면
Then [다시 시도] 액션 버튼이 포함되어야 한다
And 자동 닫힘 없이 사용자 액션 대기해야 한다
```

---

### 10.14 S-401: 정산표 (인라인 영역)

#### 10.14.1 영역 목적

메인 화면 하단에서 월별 청구 내역과 합계를 표시.
...
#### 10.14.3 행 종류 (Row Types)

| 종류 | 표시 | 정렬 |
|---|---|---|
| **기본 용돈** | "💰 기본 용돈" + 계산식 + 금액 | 항상 첫 행 |
| **학교 버스** | "🏫 학교 버스" + 계산식 + 금액 | 학교 있을 때만 |
| **학원 버스** | "📚 학원 버스" + 계산식 + 금액 | 학원 있을 때만 |
| **임시 항목 (카테고리별)** | 아이콘 + 카테고리 + 건수 + 금액 | 카테고리별 그룹 |
...
function buildSummaryRows(
  ...
  if (calc.base_allowance > 0) {
    rows.push({
      ...
      icon: "💰",
      label: "기본 용돈",
      formula: `${formatNumber(calc.base_allowance)} × 1`,
  ...
  if (calc.school_total > 0) {
    ...
    rows.push({
      ...
      icon: "🏫",
      label: "학교 버스",
      formula: `${formatNumber(settings.school.fare)} × ${multiplier} × ${calc.school_days_count}일`,
  ...
  if (calc.academy_total > 0) {
    ...
    rows.push({
      ...
      icon: "📚",
      label: "학원 버스",
      formula: `${formatNumber(settings.academy.fare)} × ${multiplier} × ${calc.academy_days_count}일`,
  ...
  const categoryMap = new Map<string, { items: ExtraItem[]; total: number }>();
  allExtraItems.forEach(item => {
    const entry = categoryMap.get(item.category) ?? { items: [], total: 0 };
    entry.items.push(item);
    entry.total += item.amount;
    categoryMap.set(item.category, entry);
  });

  Array.from(categoryMap.entries()).forEach(([category, group]) => {
    rows.push({
      type: "extra",
...
#### 10.14.5 빈 상태 (Empty State)

```jsx
{rows.length === 0 ? (
  <div className="summary-empty">
    <div className="emoji">📭</div>
    <div className="title">이번 달 청구할 항목이 없습니다</div>
...
### 10.15 S-402: 캘린더 셀 호버 툴팁

#### 10.15.1 영역 목적

PC에서 셀 호버 시 / 모바일에서 길게 누름 시 셀 상세 정보 표시.
...
##### 표시 트리거
- **PC**: 마우스가 셀 위에 200ms 이상 머무를 때
- **모바일**: 길게 누름 (long press, 500ms)

##### 위치 결정
- 셀 우측 (PC)
- 화면 우측 끝이면 좌측으로
- 모바일은 셀 위 또는 화면 중앙
- 화면 밖으로 나가지 않도록 자동 조정 (boundary detection)
...
##### 클릭 시 동작
- 툴팁이 표시 중이어도 클릭하면 S-103 셀 편집 모달로 이동
...
.cell-tooltip {
  ...
  max-width: 280px;
  font-size: var(--font-size-sm);
  pointer-events: none;  /* 호버 영역에 영향 X */
...
---
## 11. 1단계 관리자 기능 (Admin / Diagnostic)

### 11.1 관리자 기능 도입 배경

1단계는 본인 사용이 주 목적이지만, 다음 상황에서 관리자 기능이 필요:

- 데이터 손상 진단 (개발자 도구 없이도)
- 백업 파일 만들기 (브라우저 변경 대비)
- 다른 기기로 데이터 이전 (데스크톱 → 폰)
- 모든 데이터 초기화 (테스트, 양도)
- 사용자 정의 카테고리 관리
- 향후 2단계 마이그레이션 준비
...
##### 방법 1. URL 파라미터
```
https://hex.github.io/allowance-mvp/?admin=1
```

##### 방법 2. 콘솔 명령
```javascript
window.openAdmin()  // 어떤 화면에서든 호출 가능
```

##### 방법 3. 시크릿 단축키
- 헤더의 자녀 이름 영역을 빠르게 5번 탭/클릭

#### 11.2.2 진입 안전장치

- 일반 사용자가 실수로 들어와도 데이터 변경하지 않는 한 안전
- 모든 파괴적 작업(데이터 삭제 등)은 명시적 확인 필수
- "일반 모드로 돌아가기" 버튼 항상 표시
...
│ ✅ meta_v1: 정상 (192 bytes)                                       │
│ ✅ custom_categories_v1: 2개 (320 bytes)                           │
│                                                                    │
│ 캘린더 데이터:                                                      │
│ ✅ 2026-05: 정상 (2,150 bytes, 임시항목 3개)                        │
│ ✅ 2026-04: 정상 (1,820 bytes, 임시항목 2개)                        │
│ ⚠️ 2026-03: 손상 [복구] [삭제]                                     │
│                                                                    │
│ 백업 파일:                                                          │
│ • settings_v1_corrupted_1745832023456 (488 bytes) [보기] [삭제]    │
│                                                                    │
│ ── 스토리지 사용량 ──                                               │
│ 사용: 약 14 KB / 전체 5 MB (0.28%)                                 │
│                                                                    │
│ ── 작업 ──                                                          │
│                                                                    │
│ 📤 [데이터 내보내기]  → S-113                                       │
...
interface SystemInfo {
  ...
  firstUsedAt: string;         // ISO date
  lastUsedAt: string;          // ISO date
  daysSinceFirstUse: number;
  buildDate: string;           // 빌드 시점
  userAgent: string;           // 브라우저 정보
}

function getSystemInfo(): SystemInfo {
  const meta = loadMeta();
  const firstDate = new Date(meta.first_used_at);
  const today = new Date();
...
interface DataStatus {
  ...
  valid: boolean;
  size: number;             // bytes
  itemCount?: number;       // 캘린더면 임시 항목 개수
  errorMessage?: string;
}
...
function checkDataStatus(): DataStatus[] {
  ...
  // settings_v1 검사
  status.push(checkKey("settings_v1"));

  // meta_v1 검사
  status.push(checkKey("meta_v1"));

  // custom_categories_v1 검사
  status.push(checkKey("custom_categories_v1"));
  ...
  Object.keys(localStorage)
    ...
    .sort()
    .reverse()
    .forEach(key => status.push(checkKey(key)));

  // 백업 파일 검사
  Object.keys(localStorage)
    .filter(k => k.includes("_corrupted_"))
    .forEach(key => status.push({
      key,
      valid: true,  // 백업은 검증 안 함
...
function checkKey(key: string): DataStatus {
  const raw = localStorage.getItem(key);
  if (!raw) return { key, valid: false, size: 0, errorMessage: "키 없음" };
  ...
  try {
    ...
    if (key.startsWith("calendar_v1_")) {
      ...
      itemCount = cells.reduce((sum: number, c: any) => sum + (c.extra_items?.length || 0), 0);
    }

    if (key === "custom_categories_v1") {
...
"모든 데이터가 삭제됩니다. 계속하시겠습니까?"
  ↓
확인 모달 2차 (Type-to-confirm):
"확인을 위해 다음을 입력하세요: '모든 데이터 삭제'"
[입력 필드] 정확히 입력 시 [확인] 활성
  ↓
초기화 실행:
- localStorage.clear()
- 페이지 리로드
...
async function resetAllData(): Promise<void> {
  // 1. 모든 키 삭제 (시스템 키만 식별해서 삭제)
  const keysToDelete = [
    "settings_v1",
    "custom_categories_v1",
    "meta_v1",
    ...Object.keys(localStorage).filter(k => k.startsWith("calendar_v1_")),
    ...Object.keys(localStorage).filter(k => k.includes("_corrupted_"))
  ];

  keysToDelete.forEach(k => localStorage.removeItem(k));

  // 2. 페이지 리로드 → S-101 환영 모달 자동 표시
...
##### [콘솔 로그 보기]
- 마지막 50개 console.log/warn/error 로그를 화면에 표시
- 시스템: 시작 시 console을 wrapping해서 메모리에 로그 보관

##### [데이터 무결성 검사]
- 모든 데이터 키를 순회하며 스키마 검증
- 결과를 표 형태로 표시 (각 키별 정상/손상)

##### [공휴일 데이터 검증]
- holidays.json을 fetch해서 검증
- 2026~2030년 데이터 완성도 검사
- 누락된 공휴일 있으면 안내

```typescript
function validateHolidaysData(): ValidationResult {
  const expectedYears = [2026, 2027, 2028, 2029, 2030];
  const issues: string[] = [];

  expectedYears.forEach(year => {
    // 각 년도에 최소 15개 공휴일 있어야 함 (대체공휴일 포함 평균)
    const yearHolidays = Object.keys(holidays).filter(d => d.startsWith(`${year}-`));
    if (yearHolidays.length < 15) {
      issues.push(`${year}년: 공휴일 ${yearHolidays.length}개 (예상 15+)`);
    }

    // 신정, 어린이날, 광복절 등 필수 공휴일 검사
    const required = [
      `${year}-01-01`,  // 신정
      `${year}-03-01`,  // 삼일절
      `${year}-05-05`,  // 어린이날
...
│ ┌──────────────────────────────┐    │
│ │ 파일명:                       │    │
│ │ allowance-backup-2026-05-01.json    │
│ │                              │    │
│ │ 크기: 약 14 KB                │    │
│ │ 내용:                         │    │
│ │ - settings (1)                │    │
...
interface ExportFile_v1 {
  ...
  calendars: {
    [yearMonth: string]: CalendarMonth_v1;  // "2026-05" 형식
  };

  // 백업 (옵션)
  backups?: {
    [key: string]: string;       // 백업 키 → 원본 JSON 문자열
  };
...
```json
{
  ...
  "custom_categories": {
    "categories": [
      { "id": "cat_a3f9k2", "name": "동아리 회비", "icon": "🎵", "created_at": "..." }
    ],
    "version": 1
  },

  "calendars": {
    "2026-04": { "year": 2026, "month": 4, "cells": { ... }, "version": 1 },
    "2026-05": { ... }
  },

  "checksum": "a8f5b3c1d9e7..."
}
```
...
async function exportData(options: ExportOptions): Promise<Blob> {
  const data: ExportFile_v1 = {
    export_version: 1,
    exported_at: new Date().toISOString(),
    app_version: APP_VERSION,
    schema_version: 1,

    settings: options.includeSettings ? loadSettings() : undefined,
    meta: options.includeMeta ? loadMeta() : undefined,
    custom_categories: options.includeCategories ? loadCustomCategoriesRaw() : undefined,
    calendars: {},
  ...
  if (options.includeCalendars) {
    const calendarKeys = Object.keys(localStorage)
      .filter(k => k.startsWith("calendar_v1_"));
    ...
    calendarKeys.forEach(key => {
      ...
      if (options.dateRange &&
          (yearMonth < options.dateRange.start || yearMonth > options.dateRange.end)) {
        return;
      }

      const calendarData = JSON.parse(localStorage.getItem(key) || "{}");
      data.calendars[yearMonth] = calendarData;
    });
  }
  ...
  if (options.includeBackups) {
    ...
    Object.keys(localStorage)
      .filter(k => k.includes("_corrupted_"))
      .forEach(k => {
        data.backups![k] = localStorage.getItem(k) || "";
      });
  }

  // 체크섬 계산
  const dataString = JSON.stringify(data, null, 2);
  data.checksum = await computeSHA256(dataString);

  // 최종 JSON 문자열 (체크섬 포함)
  const finalString = JSON.stringify(data, null, 2);
...
function getDefaultFilename(): string {
  ...
  return `allowance-backup-${dateStr}.json`;
}
```
...
```typescript
async function downloadExport(blob: Blob, filename: string): Promise<void> {
...
### 11.5 S-114: 데이터 가져오기 모달

#### 11.5.1 화면 목적

내보낸 JSON 파일을 다시 가져와서 데이터 복원/병합.
...
├──────────────────────────────────────┤
│                                      │
│ 파일: allowance-backup-2026-05-01.json│
│ ✅ 유효한 백업 파일                   │
│ ✅ 체크섬 검증 통과                   │
│                                      │
│ 내보낸 시간: 2026-05-01 10:00        │
│ 앱 버전: 1.0.0 (호환)                │
│                                      │
│ ── 가져올 데이터 ──                   │
│ ☑ 자녀 기본 설정                     │
│ ☑ 사용자 정의 카테고리 (2개)          │
│ ☑ 캘린더 데이터 (3개월: 2026-04~06)  │
│                                      │
│ ── 복원 모드 ──                       │
│ ● 덮어쓰기 (기존 데이터 모두 교체)     │
│ ○ 병합 (없는 데이터만 추가)           │
│                                      │
│ ⚠️ 덮어쓰기를 선택하면 현재 데이터가  │
│    사라집니다.                        │
│                                      │
│ 백업 권장: [먼저 현재 데이터 내보내기] │
│                                      │
├──────────────────────────────────────┤
│         [취소]    [📥 가져오기]     │
└──────────────────────────────────────┘
...
```typescript
async function importData(file: File, mode: "overwrite" | "merge"): Promise<ImportResult> {
  // 1. 파일 읽기
  const text = await file.text();

  // 2. JSON 파싱
  let data: ExportFile_v1;
  try {
  ...
  data.checksum = "";  // 임시 비움 (계산 시)
  const computedChecksum = await computeSHA256(JSON.stringify(data, null, 2));
  ...
  // 6. 적용
  const result: ImportResult = { success: true, applied: { settings: 0, categories: 0, calendars: 0 } };
  ...
  if (data.settings) {
    if (mode === "overwrite" || !localStorage.getItem("settings_v1")) {
  ...
  if (data.custom_categories) {
    let merged: CustomCategory[] = [];
    if (mode === "merge") {
      merged = loadCustomCategories();
    }

    data.custom_categories.categories.forEach(cat => {
  ...
  if (data.calendars) {
    Object.entries(data.calendars).forEach(([yearMonth, calendar]) => {
      const [year, month] = yearMonth.split("-").map(Number);
      const key = `calendar_v1_${year}_${pad(month)}`;
...
| 에러 | 메시지 |
|---|---|
| INVALID_JSON | "파일을 읽을 수 없습니다. 백업 파일이 손상되었거나 다른 형식일 수 있습니다." |
| INVALID_SCHEMA | "백업 파일 형식이 다릅니다. 다른 시스템의 파일일 수 있습니다." |
| CHECKSUM_MISMATCH | "파일 무결성 검증 실패. 파일이 변조되었거나 손상되었습니다." |
| VERSION_MISMATCH | "백업 파일 버전이 호환되지 않습니다. (파일: vX, 시스템: v1)" |

---

### 11.6 S-203: 카테고리 관리 드로어

#### 11.6.1 화면 목적

사용자 정의 카테고리의 추가/수정/삭제.
...
[🗑] 클릭 → 확인 모달
"이 카테고리를 삭제할까요?
사용 중인 임시 항목 X건이 영향받습니다.
- 사용 중이면 임시 항목은 그대로 유지 (이름 보존)"
[취소] / [삭제]
```
...
```typescript
function getCategoryUsage(): Map<string, number> {
  ...
  Object.keys(localStorage)
    ...
    .forEach(key => {
      ...
      Object.values(calendar.cells || {}).forEach((cell: any) => {
        (cell.extra_items || []).forEach((item: any) => {
...
```typescript
function cleanupUnusedCategories(): { deleted: string[] } {
  const usage = getCategoryUsage();
  const customCategories = loadCustomCategories();

  const unused = customCategories.filter(cat => !usage.has(cat.name));
  const remaining = customCategories.filter(cat => usage.has(cat.name));

  saveCustomCategories(remaining);

  return { deleted: unused.map(c => c.name) };
}
```
...
| ID | 라벨 | 위치 | 동작 | 활성 조건 | 비활성 조건 | 키보드 | aria-label |
|---|---|---|---|---|---|---|---|
| BTN-H-001 | ◀ | 헤더 좌측 | 이전 달 이동 | 항상 | 없음 (과거 무제한) | ← 또는 Enter | "이전 달" |
| BTN-H-002 | 월 표시 | 헤더 중앙 | S-202 월 선택 드롭다운 | 항상 | 없음 | Enter | "월 선택" |
| BTN-H-003 | ▶ | 헤더 우측 | 다음 달 이동 | 미래 +12개월 미만 | 미래 +12개월 도달 | → 또는 Enter | "다음 달" |
| BTN-H-004 | ⚙ | 헤더 우측 | S-102 설정 모달 | 항상 | 없음 | Enter | "설정" |
| BTN-H-005 | 📌 | 헤더 우측 | S-201 비고 토글 | 항상 | 없음 | Enter | "비고" |
| BTN-H-006 | 📋 | 헤더 우측 (모바일은 하단) | 메시지 복사 | 합계 > 0 | 합계 = 0 | Enter | "메시지 복사" |

#### 12.1.2 캘린더 셀

| ID | 영역 | 동작 | 활성 조건 | 비활성 조건 | 키보드 |
|---|---|---|---|---|---|
| BTN-C-001 | 캘린더 셀 | S-103 셀 편집 모달 | 해당 월의 모든 날짜 | placeholder 셀 | Enter (포커스 시) |

#### 12.1.3 셀 편집 모달 (S-103)

| ID | 라벨 | 동작 | 활성 조건 | 비활성 조건 |
|---|---|---|---|---|
| BTN-CE-001 | + 임시 항목 추가 | S-104 폼 모달 | 임시 항목 < 3 | 임시 항목 = 3 |
| BTN-CE-002 | ✏️ (수정) | S-104 폼 (수정 모드) | 항상 | - |
| BTN-CE-003 | 🗑 (삭제) | S-107 삭제 확인 | 항상 | - |
| BTN-CE-004 | 취소 | 변경 검사 → S-106 또는 닫기 | 항상 | - |
| BTN-CE-005 | 저장 | 검증 → 저장 → 닫기 | 변경 있음 | 변경 없음 |
| BTN-CE-006 | × (헤더) | BTN-CE-004와 동일 | 항상 | - |

#### 12.1.4 임시 항목 폼 (S-104)

| ID | 라벨 | 동작 | 활성 조건 | 비활성 조건 |
|---|---|---|---|---|
| BTN-EF-001 | 카테고리 드롭다운 | 카테고리 목록 표시 | 항상 | - |
| BTN-EF-002 | + 새 카테고리 (드롭다운 내) | S-105 새 카테고리 모달 | 항상 | - |
| BTN-EF-003 | 추가 / 저장 | 검증 → 부모 모달에 반영 | 모든 필드 입력 | 검증 실패 |
| BTN-EF-004 | 취소 | S-104 닫기 (변경 폐기) | 항상 | - |
| BTN-EF-005 | × (헤더) | BTN-EF-004와 동일 | 항상 | - |

#### 12.1.5 새 카테고리 모달 (S-105)

| ID | 라벨 | 동작 |
|---|---|---|
| BTN-NC-001 | 이모지 그리드 (각 이모지) | 선택 (단일 선택) |
| BTN-NC-002 | 추가 | 검증 → 저장 → 닫기 |
| BTN-NC-003 | 취소 | 닫기 |
...
| ID | 라벨 | 동작 | 활성 조건 |
|---|---|---|---|
| BTN-S-001 | 요일 토글 (각 요일) | 선택/해제 | 항상 |
| BTN-S-002 | 편도/왕복 라디오 | 선택 | 항상 (학교/학원 활성 시) |
| BTN-S-003 | 공휴일 처리 라디오 | 선택 | 항상 |
| BTN-S-004 | 저장 | 검증 → 저장 → 닫기 | 검증 통과 |
| BTN-S-005 | 취소 | 변경 검사 → S-106 또는 닫기 | S-102만 |
...
| BTN-N-001 | × (헤더) | 닫기 |
| BTN-N-002 | 외부 백드롭 | 닫기 |
| BTN-N-003 | 모바일 스와이프 | 닫기 |

#### 12.1.8 시스템 모달

| 모달 | 버튼 | 동작 |
|---|---|---|
| S-106 변경 확인 | [계속 편집] | S-106 닫기, 부모 모달 유지 |
| S-106 변경 확인 | [닫기] | S-106 + 부모 모달 모두 닫기 |
| S-107 삭제 확인 | [취소] | S-107 닫기 |
| S-107 삭제 확인 | [삭제] | 항목 제거, S-107 닫기 |
| S-108 정리 안내 | [취소] | 닫기 |
| S-108 정리 안내 | [지금 정리] | 정리 실행 |
| S-108 정리 안내 | [내보내기 후 정리] | S-113 → 정리 |
| S-109 손상 안내 | [다시 설정하기] | S-101 강제 표시 |
| S-111 폴백 | [모두 선택] | textarea 텍스트 선택 |
| S-111 폴백 | [닫기] | 닫기 |
...
| ID | 라벨 | 동작 |
|---|---|---|
| BTN-A-001 | 일반 모드로 돌아가기 | URL 변경 → S-001 |
| BTN-A-002 | 데이터 내보내기 | S-113 |
| BTN-A-003 | 데이터 가져오기 | S-114 |
| BTN-A-004 | 카테고리 관리 | S-203 |
| BTN-A-005 | 오래된 데이터 정리 | 확인 모달 → 정리 |
| BTN-A-006 | 모든 데이터 초기화 | 2단계 확인 → clear |
| BTN-A-007 | 콘솔 로그 보기 | 로그 화면 표시 |
| BTN-A-008 | 데이터 무결성 검사 | 검사 실행 + 결과 표시 |
| BTN-A-009 | 공휴일 데이터 검증 | 검증 실행 + 결과 표시 |
| BTN-A-010 | [복구] (각 손상 캘린더) | 복구 시도 |
| BTN-A-011 | [삭제] (각 손상 캘린더) | 확인 후 삭제 |
| BTN-A-012 | [보기] (각 백업 파일) | 콘솔에 출력 |
| BTN-A-013 | [삭제] (각 백업 파일) | 확인 후 삭제 |
...
| Enter | 활성 요소 클릭 | 항상 |
| Space | 체크박스/버튼 토글 | 항상 |
| ESC | 가장 위 모달 닫기 | 모달 열림 시 |
| ← | 이전 달 | 메인 화면 (입력 외) |
| → | 다음 달 | 메인 화면 (입력 외) |

#### 12.2.2 셀 그리드 네비게이션 (선택, 1단계 후반)

| 키 | 동작 |
|---|---|
| ↑ ↓ ← → | 인접 셀로 포커스 이동 |
| Enter | 선택된 셀 편집 (S-103) |
| Home | 월 첫째 날 |
| End | 월 마지막 날 |
| Page Up | 이전 달 |
| Page Down | 다음 달 |
...
|---|---|
| Tab | 모달 내 포커스 트랩 |
| Enter | 폼 [저장] 버튼 (default) |
| ESC | 모달 닫기 (변경 검사 적용) |

---

### 12.3 마우스/터치 인터랙션 매트릭스

#### 12.3.1 호버 / 터치 이벤트

| 영역 | Hover (PC) | Click (PC) | Tap (Mobile) | Long Press (Mobile) |
|---|---|---|---|---|
| 캘린더 셀 (값 있음) | 배경 강조 + S-402 툴팁 | S-103 모달 | S-103 모달 | S-402 툴팁 |
| 캘린더 셀 (빈 평일) | 배경 강조 (셀 가능 표시) | S-103 모달 (메모용) | S-103 모달 | - |
| 캘린더 셀 (placeholder) | 동작 없음 | 동작 없음 | 동작 없음 | - |
| 정산표 행 (학교/학원) | 강조 | 동작 없음 | 동작 없음 | - |
| 정산표 행 (임시 카테고리) | 강조 | 첫 항목 셀로 스크롤 (선택) | 동일 | - |
| 헤더 버튼 | 배경 강조 | 해당 동작 | 해당 동작 | - |
| 모달 외부 | - | 닫기 (모달별 정책) | 동일 | - |
...
|---|---|
| 비고 바텀시트 | 아래로 스와이프 → 닫기 |
| 월 선택 바텀시트 | 아래로 스와이프 → 닫기 |
| 캘린더 (메인) | 좌우 스와이프 → 월 이동 (선택, 1.1) |
| 임시 항목 카드 | 좌측 스와이프 → 삭제 액션 (선택, 1.1) |
...
```typescript
function getCellAriaLabel(cell: CellData): string {
  const parts: string[] = [];
  ...
  // 공휴일
  if (cell.isHoliday) {
    parts.push(`공휴일 ${cell.holidayName}`);
  }

  // 학교
  if (cell.school_fee > 0) {
    parts.push(`학교 등교 ${cell.school_fee.toLocaleString()}원`);
  }

  // 학원
  if (cell.academy_fee > 0) {
    parts.push(`학원 등원 ${cell.academy_fee.toLocaleString()}원`);
  }

  // 임시 항목
  if (cell.extra_items.length > 0) {
    const total = cell.extra_items.reduce((s, i) => s + i.amount, 0);
    parts.push(`임시 항목 ${cell.extra_items.length}건 ${total.toLocaleString()}원`);
  }
  ...
  // 합계
  if (cell.total > 0) {
    parts.push(`합계 ${cell.total.toLocaleString()}원`);
  }
...
>
  ...
  <p id="modal-description" className="sr-only">
    이 셀의 임시 항목과 메모를 추가하거나 수정할 수 있습니다.
  </p>
  {/* 모달 내용 */}
</div>
```
...
| #1565C0 (primary) on #FFFFFF | 7.8:1 ✅ |
| #FFFFFF on #1565C0 | 7.8:1 ✅ |
| #C0392B (holiday) on #FFFFFF | 5.4:1 ✅ |
...
<button onClick={handleCopy}>
  ...
  <span className="sr-only">메시지 복사</span>
  복사
</button>
```
...
| 화면 ID | < 768px (Mobile) | 768~1023px (Tablet) | 1024px+ (Desktop) |
|---|---|---|---|
| S-001 | 세로, 셀 60px | 세로, 셀 80px | 세로, 셀 90px (1440+ 우측 사이드바) |
| S-101/S-102 | 풀스크린 | 480px 모달 | 동일 |
| S-103 | 95% 폭 | 480px 모달 | 동일 |
| S-104 | 95% 폭 | 400px 모달 | 동일 |
| S-105 | 95% 폭 | 400px 모달 | 동일 |
| S-106/S-107 | 90% 폭, 360px max | 360px 모달 | 동일 |
| S-108/S-109 | 90% 폭 | 480px 모달 | 동일 |
| S-201 | 바텀시트 | 우측 드로어 320px | 동일 |
| S-202 | 바텀시트 | 팝오버 | 팝오버 |
| S-301~303 | 상단 토스트 | 우측 하단 토스트 | 동일 |
| S-112 | 풀스크린 | 풀스크린 | 큰 컨테이너 (1024px max) |

#### 12.5.2 컴포넌트별 반응형

| 컴포넌트 | Mobile | Desktop |
|---|---|---|
| CurrencyInput | 폭 100%, 키보드 numeric | 폭 200px, 키보드 일반 |
| WeekdayPicker | 줄바꿈 가능 | 가로 배치 |
| RadioGroup | 세로 (2열) | 가로 |
| Button | 폭 100% (footer) | auto |

---

## 13. v2.1 변경 이력

### 13.1 v2.0 → v2.1 변경 내용

#### 추가 챕터
- **9. 1단계 화면 시스템** (Screen System)
  - 화면 인벤토리 (총 17개 화면)
  - 정보 구조 / IA 다이어그램
  - 화면 전이 매트릭스
  - 네비게이션 정책 (모달 닫기 규칙, 중첩 정책, 포커스 관리)
  - 반응형 디자인 정책
  - 빈 상태 / 로딩 상태
  - Z-order 정책
...
- 디자인 토큰 (CSS 변수) 구체화
- 컴포넌트 props 명세 정확화
- 검증 함수 의사코드 → 실제 동작 가능한 TypeScript
- 모든 모달의 닫기 규칙 명세
- 인수 조건 (Gherkin) 추가

#### 새로 식별된 화면
v2.0에서 누락되거나 두루뭉실했던 화면:
- 변경 사항 확인 모달 (S-106) — 데이터 변경 추적
- 데이터 정리 안내 모달 (S-108) — 스토리지 가득 시
- 데이터 손상 안내 (S-109) — 복구 절차
- 클립보드 폴백 모달 (S-111) — 호환성
- 진단 화면 (S-112) — 관리자/디버그
- 데이터 내보내기/가져오기 (S-113/S-114) — 백업·이전
- 카테고리 관리 드로어 (S-203) — 사용자 정의 카테고리
- 셀 호버 툴팁 (S-402) — 빠른 미리보기
- 토스트 종류 분리 (S-301/S-302/S-303)

#### 새로 식별된 인터랙션
- 키보드 ↑↓←→ 셀 네비게이션
- 모바일 길게 누름 (long press)
- 모달 외부 클릭 정책
- ESC 우선순위 (LIFO)
- 포커스 트랩 / 포커스 복귀
- 스와이프 닫기 (모바일 바텀시트)
...
| 구분 | v2.0 | v2.1 | 증가 |
|---|---:|---:|---:|
| 1단계 PRD 분량 | 약 4,500줄 | 약 8,000줄 | +3,500줄 |
| 화면 정의 수 | 6개 (대략) | 17개 (상세) | +11개 |
| 버튼 명세 수 | 0 | 50+ | +50+ |
| 인수 조건 (Gherkin) | 약 80개 | 약 130개 | +50개 |
| TypeScript 의사코드 | 약 30개 함수 | 약 70개 함수 | +40개 |
...
| v2.1 | 2026-05-01 | 1단계의 두루뭉실한 화면·조회·설정 영역 면밀화. 17개 화면 인벤토리, 화면별 상세 명세, 관리자/진단 모드, 50+ 버튼 카탈로그, 인터랙션·접근성 매트릭스 추가. |
